...
class SOasisContentBrowser;
class FSpawnTabArgs;
class FTabManager;
class FViewport;
class UFactory;

struct FOasisContentBrowserPluginInfo;
...
struct FOasisContentBrowserPluginInfo
{
	FOasisContentBrowserPluginInfo();
 ...
 */
class FOasisContentBrowserSingleton : public IOasisContentBrowserSingleton
{
public:
	/** Constructor, Destructor */
	FOasisContentBrowserSingleton();
	virtual ~FOasisContentBrowserSingleton();

	// IOasisContentBrowserSingleton interface
	...
	virtual void SyncBrowserToAssets(const TArray<struct FAssetData>& AssetDataList, bool bAllowLockedBrowsers = false, bool bFocuSOasisContentBrowser = true, const FName& InstanceName = FName(), bool bNewSpawnBrowser = false) override;
	virtual void SyncBrowserToAssets(const TArray<UObject*>& AssetList, bool bAllowLockedBrowsers = false, bool bFocuSOasisContentBrowser = true, const FName& InstanceName = FName(), bool bNewSpawnBrowser = false) override;
	virtual void SyncBrowserToFolders(const TArray<FString>& FolderList, bool bAllowLockedBrowsers = false, bool bFocuSOasisContentBrowser = true, const FName& InstanceName = FName(), bool bNewSpawnBrowser = false) override;
	virtual void SyncBrowserTo(const FOasisContentBrowserSelection& ItemSelection, bool bAllowLockedBrowsers = false, bool bFocuSOasisContentBrowser = true, const FName& InstanceName = FName(), bool bNewSpawnBrowser = false) override;
#endif
	virtual void GetSelectedAssets(TArray<FOasisAssetData>& SelectedAssets) override;
#if ECB_LEGACY
	...
	static FOasisContentBrowserSingleton& Get();
	...
	void SetPrimaryContentBrowser(const TSharedRef<SOasisContentBrowser>& NewPrimaryBrowser);

#if ECB_WIP_MULTI_INSTANCES
	/** Notifies the singleton that a browser was closed */
	void ContentBrowserClosed(const TSharedRef<SOasisContentBrowser>& ClosedBrowser);
...
private:
	...
	void OpenSpyEngineButtonClicked() const;

#if ECB_LEGACY
	/** Util to get or create the content browser that should be used by the various Sync functions */
	TSharedPtr<SOasisContentBrowser> FindContentBrowserToSync(bool bAllowLockedBrowsers, const FName& InstanceName = FName(), bool bNewSpawnBrowser = false);
	...
	void FocuSOasisContentBrowser(const TSharedPtr<SOasisContentBrowser>& BrowserToFocus);
