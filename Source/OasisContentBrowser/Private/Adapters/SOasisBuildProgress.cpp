...
SOasisBuildProgressWidget::SOasisBuildProgressWidget() 
{			
}
...
void SOasisBuildProgressWidget::Construct( const FArguments& InArgs )
{
	ChildSlot
	[
		SNew(SBorder)
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(this, &SOasisBuildProgressWidget::OnGetBuildTimeText)
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				...
				[
					SNew(STextBlock)
					.Text(this, &SOasisBuildProgressWidget::OnGetProgressText)
				]
			]
			+ SVerticalBox::Slot()
			.HAlign(HAlign_Fill)
			...
			[
				SNew(SProgressBar)
				.Percent(this, &SOasisBuildProgressWidget::OnGetProgressFraction)
			]
			+ SVerticalBox::Slot()
			.Padding(15.0f, 10.0f)
			.HAlign(HAlign_Right)
			...
			.OnClicked(this, &SOasisBuildProgressWidget::OnStopBuild)
			]
		]
	];
...
FText SOasisBuildProgressWidget::OnGetProgressText() const
{
	return ProgressStatusText;
}

void SOasisBuildProgressWidget::UpdateProgressText()
{
...
FText SOasisBuildProgressWidget::OnGetBuildTimeText() const
{
	// Only show a percentage if there is something interesting to report
	return BuildStatusTime; 
}

TOptional<float> SOasisBuildProgressWidget::OnGetProgressFraction() const
{
	// Only show a percentage if there is something interesting to report
	if( ProgressNumerator > 0 && ProgressDenominator > 0 )
	{
...
void SOasisBuildProgressWidget::SetBuildType(EBuildType InBuildType)
{
	BuildType = InBuildType;
}

FText SOasisBuildProgressWidget::BuildElapsedTimeText() const
{
	// Display elapsed build time.
	return FText::AsTimespan( FDateTime::Now() - BuildStartTime );
}

void SOasisBuildProgressWidget::UpdateTime()
{
	BuildStatusTime = BuildElapsedTimeText();
}

void SOasisBuildProgressWidget::SetBuildStatusText( const FText& StatusText )
{
	UpdateTime();
...
void SOasisBuildProgressWidget::SetBuildProgressPercent( int32 InProgressNumerator, int32 InProgressDenominator )
{
	UpdateTime();
...
void SOasisBuildProgressWidget::MarkBuildStartTime()
{
	BuildStartTime = FDateTime::Now();
}

FReply SOasisBuildProgressWidget::OnStopBuild()
{
