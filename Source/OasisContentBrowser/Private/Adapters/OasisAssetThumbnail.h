...
struct FOasisAssetThumbnailConfig
{
	FOasisAssetThumbnailConfig()
		: bAllowFadeIn( false )
...
class FOasisAssetThumbnail
	...
	, public TSharedFromThis<FOasisAssetThumbnail>
{
public:
	/**
	 ...
	 */
	FOasisAssetThumbnail( UObject* InAsset, uint32 InWidth, uint32 InHeight, const TSharedPtr<class FOasisAssetThumbnailPool>& InThumbnailPool );
	FOasisAssetThumbnail( const FOasisAssetData& InAsset, uint32 InWidth, uint32 InHeight, const TSharedPtr<class FOasisAssetThumbnailPool>& InThumbnailPool );
	~FOasisAssetThumbnail();
	 ...
	 */
	const FOasisAssetData& GetAssetData() const;
	 ...
	 */
	void SetAsset( const FOasisAssetData& InAssetData );

	/**
	 * @return A slate widget representing this thumbnail
	 */
	TSharedRef<SWidget> MakeThumbnailWidget( const FOasisAssetThumbnailConfig& InConfig = FOasisAssetThumbnailConfig() );

	/** Re-renders this thumbnail */
	void RefreshThumbnail();

	DECLARE_EVENT(FOasisAssetThumbnail, FOnAssetDataChanged);
...
private:
	...
	TWeakPtr<FOasisAssetThumbnailPool> ThumbnailPool;
 ...
 */
class FOasisAssetThumbnailPool : public FTickableEditorObject
{
public:
	 ...
	 */
	FOasisAssetThumbnailPool( uint32 InNumInPool, const TAttribute<bool>& InAreRealTimeThumbnailsAllowed = true, double InMaxFrameTimeAllowance = 0.005, uint32 InMaxRealTimeThumbnailsPerFrame = 3 );

	/** Destructor to free all remaining resources */
	~FOasisAssetThumbnailPool();
	 ...
	 */
	FSlateTexture2DRHIRef* AccessTexture( const FOasisAssetData& AssetData, uint32 Width, uint32 Height );

	/**
	 * Adds a referencer to keep textures around as long as they are needed
	 */
	void AddReferencer( const FOasisAssetThumbnail& AssetThumbnail );

	/**
	 * Removes a referencer to clean up textures that are no longer needed
	 */
	void RemoveReferencer( const FOasisAssetThumbnail& AssetThumbnail );

	/** Returns true if the thumbnail for the specified asset in the specified size is in the stack of thumbnails to render */
	bool IsInRenderStack( const TSharedPtr<FOasisAssetThumbnail>& Thumbnail ) const;

	/** Returns true if the thumbnail for the specified asset in the specified size has been rendered */
	bool IsRendered( const TSharedPtr<FOasisAssetThumbnail>& Thumbnail ) const;

	/** Brings all items in ThumbnailsToPrioritize to the front of the render stack if they are actually in the stack */
	void PrioritizeThumbnails( const TArray< TSharedPtr<FOasisAssetThumbnail> >& ThumbnailsToPrioritize, uint32 Width, uint32 Height );

	/** Register/Unregister a callback for when thumbnails are rendered */
	DECLARE_EVENT_OneParam( FOasisAssetThumbnailPool, FThumbnailRendered, const FOasisAssetData& );
	...
	DECLARE_EVENT_OneParam( FOasisAssetThumbnailPool, FThumbnailRenderFailed, const FOasisAssetData& );
	FThumbnailRenderFailed& OnThumbnailRenderFailed() { return ThumbnailRenderFailedEvent; }

	/** Re-renders the specified thumbnail */
	void RefreshThumbnail( const TSharedPtr<FOasisAssetThumbnail>& ThumbnailToRefresh );
