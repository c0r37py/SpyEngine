...
#include "Widgets/SOasisPathView.h"
...
void FPathContextMenu::MakePathViewContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	if ( NumAssetPaths > 0 )
	{
		...
		MenuBuilder.BeginSection("PathViewFolderOptions", LOCTEXT("PathViewOptionsMenuHeading", "Folder Options") );
		{
			...
			// Explore
			MenuBuilder.AddMenuEntry(
				OasisContentBrowserUtils::GetExploreFolderText(),
	...
	MenuBuilder.BeginSection("PathViewProjectOptions", LOCTEXT("PathViewProjectOptionsMenuHeading", "Project"));
	{
		if (CanExecuteApplyProjectFolderColors())
		{
			...
			MenuBuilder.AddMenuEntry(
				...
				LOCTEXT("ImportProjectColorsTooltip", "Import folder colors from selected project in SpyEngine to current project.\n (Will override current project's folder color setting)"),
				FSlateIcon(),
				FUIAction(
...
bool FPathContextMenu::CanCreateAsset() const
{
	...
	return SelectedPaths.Num() == 1 && !OasisContentBrowserUtils::IsClassPath(SelectedPaths[0]);
}
...
void FPathContextMenu::MakeSetColorSubMenu(FMenuBuilder& MenuBuilder)
{
	...
	if ( OasisContentBrowserUtils::HasCustomColors( &CustomColors ) )
	{	
...
void FPathContextMenu::ExecuteMigrateFolder()
{
	...
	if ( ensure(SourcesPath.Len()) )
	{
		...
		OasisContentBrowserUtils::GetAssetsInPaths(SelectedPaths, AssetDataList);
...
{
	TArray<FOasisAssetData*> AssetsInFolder;

	for (int32 PathIdx = 0; PathIdx < SelectedPaths.Num(); ++PathIdx)
	{
		const FString& Path = SelectedPaths[PathIdx];
		TArray<FOasisAssetData*> Assets = FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetsByFolder(*Path);
	...
	if (AssetsInFolder.Num() > 0)
	{
		...
		OasisContentBrowserUtils::NotifyMessage(ValidateResult);
	}
}
...
// 		if (FOasisContentBrowserSingleton::GetAssetRegistry().GetAssetContentRootDir(SourcesPath, AssetContentDir))
// 		{
// 			return FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetContentRootConfigDirs().Contains(*AssetContentDir);
// 		}

		TArray<FName> ContentRootHosts;
		FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetContentRootHostDirs().GenerateKeyArray(ContentRootHosts);
		...
		if (FOasisContentBrowserSingleton::GetAssetRegistry().IsRootFolder(SourcesPath)
			&& FOasisContentBrowserSingleton::GetAssetRegistry().QueryRootContentPathInfo(SourcesPath, nullptr, &ContentType)
			&& (ContentType == FOasisAssetData::EContentType::Project || ContentType == FOasisAssetData::EContentType::Plugin)
			)
		{
			return true;
		}
...
void FPathContextMenu::ExecuteRescanFolder()
{
	...
	{
		FOasisContentBrowserSingleton::GetAssetRegistry().ReGatheringFolders(SelectedPaths);
	}
}
...
void FPathContextMenu::ExecuteImportFolder()
{
	...
	if (AssetsInFolder.Num() > 0)
	{
		TArray<FOasisAssetData> AssetsToImport;
		for (FOasisAssetData* AssetPtr : AssetsInFolder)
		{
...
void FPathContextMenu::ExecuteReloadRootFolder()
{
	...
	if (FOasisContentBrowserSingleton::GetAssetRegistry().ReloadRootFolders(GetSelectedPaths(), &Reloaded))
	{
		...
		OasisContentBrowserUtils::NotifyMessage(FText::FromString(ReloadMessage), false, 3.f);
	}
}
...
void FPathContextMenu::ExecuteRemoveRootFolder()
{
	...
	if (FOasisContentBrowserSingleton::GetAssetRegistry().RemoveRootFolders(GetSelectedPaths(), &Removed))
	{
		...
		OasisContentBrowserUtils::NotifyMessage(FText::FromString(RemovedMessage), false, 3.f);
	}
}
...
void FPathContextMenu::ExecuteAddRootFolder()
{
	...
	if (DesktopPlatform)
	{
		...
		if (bFolderSelected)
		{
			...
			if (FOasisContentBrowserSingleton::GetAssetRegistry().IsRootFolder(FolderName))
			{
				FString AlreadyExist = FString::Printf(TEXT("%s already exist. skip."), *FolderName);
				OasisContentBrowserUtils::NotifyMessage(FText::FromString(AlreadyExist), false, 3.f);
				return;
			}

			TArray<FString> Added;
			TArray<FString> Combined;
			if (FOasisContentBrowserSingleton::GetAssetRegistry().AddRootFolder(FolderName, &Added, &Combined))
			{
				...
				OasisContentBrowserUtils::NotifyMessage(FText::FromString(AddedMessage), false, 3.f);
			}
		}
	}
}
...
void FPathContextMenu::ExecuteExportRootFolderList()
{
	...
	if (DesktopPlatform)
	{
		...
		FString DefaultPath;
		FString DefaultFile(TEXT("SpyEngineFolderList.txt"));
		...
		if (OutFilenames.Num() == 1)
		{
			...
			else
			{
				if (bReplaceCurrent)
				{
					...
					FOasisContentBrowserSingleton::GetAssetRegistry().ReplaceRootContentPathsWith(FileContentLines);
				}
				else
				{
					Message = FString::Printf(TEXT("%s loaded and merged."), *OutFilenames[0]);
					FOasisContentBrowserSingleton::GetAssetRegistry().MergeRootContentPathsWith(FileContentLines, /*bReplace*/ false);
				}
			}
...
void FPathContextMenu::ExecutePickColor()
{
	...
	FLinearColor InitialColor = OasisContentBrowserUtils::GetDefaultColor();
	...
	if ( SelectedPaths.Num() > 0 )
	{
		...
		{
			const FString& Path = SelectedPaths[PathIdx];
			TSharedPtr<FLinearColor> Color = OasisContentBrowserUtils::LoadColor( Path );
			if ( Color.IsValid() )
			{
...
void FPathContextMenu::NewColorComplete(const TSharedRef<SWindow>& Window)
{
	...
	{
		const FString& Path = SelectedPaths[PathIdx];
		const TSharedPtr<FLinearColor> Color = OasisContentBrowserUtils::LoadColor( Path );
		check( Color.IsValid() );
		OasisContentBrowserUtils::SaveColor( Path, Color );		
	}
}
...
void FPathContextMenu::ResetColors()
{
	...
	{
		const FString& Path = SelectedPaths[PathIdx];
		OasisContentBrowserUtils::SaveColor( Path, NULL );		
	}
}
...
void FPathContextMenu::ExecuteSaveFolder()
{
	...
	if ( Packages.Num() )
	{
		OasisContentBrowserUtils::SavePackages(Packages);
	}
}
...
void FPathContextMenu::ExecuteDelete()
{
	...
	if (ParentContent.IsValid())
	{
		...
		FOnClicked OnYesClicked = FOnClicked::CreateSP( this, &FPathContextMenu::ExecuteDeleteFolderConfirmed );
 		OasisContentBrowserUtils::DisplayConfirmationPopup(
			Prompt,
...
void FPathContextMenu::ExecuteFixUpRedirectorsInFolder()
{
	...
	if (AssetList.Num() > 0)
	{
		...
		if (OasisContentBrowserUtils::LoadAssetsIfNeeded(ObjectPaths, Objects, bAllowedToPromptToLoadAssets, bLoadRedirects))
		{
...
{
	if ( OasisContentBrowserUtils::DeleteFolders(SelectedPaths) )
	{
		ResetColors();
...
{
	bCanExecuteRootDirsActions = FOasisContentBrowserSingleton::GetAssetRegistry().IsRootFolders(SelectedPaths);
...
bool FPathContextMenu::SelectedHasCustomColors() const
{
	for (int32 PathIdx = 0; PathIdx < SelectedPaths.Num(); ++PathIdx)
	{
		...
		const FString& Path = SelectedPaths[PathIdx];
		const TSharedPtr<FLinearColor> Color = OasisContentBrowserUtils::LoadColor( Path );
		if ( Color.IsValid() && !Color->Equals( OasisContentBrowserUtils::GetDefaultColor() ) )
		{
			return true;
		}
	}
