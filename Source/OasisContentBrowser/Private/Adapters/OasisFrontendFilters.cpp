...
public:
	...
	FName GetSourceTagFromAlias(const FOasisAssetData& InAssetData, const FName InAlias)
	{
...
FFrontendFilter_Recent::FFrontendFilter_Recent(TSharedPtr<FExtFrontendFilterCategory> InCategory)
	...
	, bIsCurrentlyActive(false)
{
	UOasisContentBrowserSettings::OnSettingChanged().AddRaw(this, &FFrontendFilter_Recent::ResetFilter);
}
...
{
#if ECB_LEGACY
	static const FName ContentBrowserName(TEXT("OasisContentBrowser"));

	RecentPackagePaths.Reset();
	FOasisContentBrowserModule& CBModule = FModuleManager::LoadModuleChecked<FOasisContentBrowserModule>(ContentBrowserName);
...
{
#if ECB_LEGACY
	if (InName == FOasisContentBrowserModule::NumberOfRecentAssetsName)
	{
		BroadcastChangedEvent();
	}
#endif
