...
FOasisPackageReader::FOasisPackageReader()
{
	Loader = nullptr;
	PackageFileSize = 0;
...
bool FOasisPackageReader::OpenPackageFile(const FString& InPackageFilename, EOpenPackageResult* OutErrorCode)
{
...
bool FOasisPackageReader::OpenPackageFile(FArchive* InLoader, EOpenPackageResult* OutErrorCode)
{
	Loader = InLoader;
	bLoaderOwner = false;
...
bool FOasisPackageReader::OpenPackageFile(EOpenPackageResult* OutErrorCode)
{
...
bool FOasisPackageReader::StartSerializeSection(int64 Offset)
{
	check(Loader);
...
bool FOasisPackageReader::ReadAssetRegistryData (TArray<FAssetData*>& AssetDataList)
{
...
bool FOasisPackageReader::ReadAssetRegistryData(FOasisAssetData& OutAssetData)
{
	if (!StartSerializeSection(PackageFileSummary.AssetRegistryDataOffset))
	{
		return false;
...
bool FOasisPackageReader::SerializeAssetRegistryDependencyData(FExtPackageDependencyData& DependencyData)
{
...
bool FOasisPackageReader::SerializePackageTrailer(FAssetPackageData& PackageData)
{
...
bool FOasisPackageReader::ReadAssetDataFromThumbnailCache(TArray<FAssetData*>& AssetDataList)
{
...
bool FOasisPackageReader::ReadAssetDataFromThumbnailCache(FOasisAssetData& OutAssetData)
{
	if (!StartSerializeSection(PackageFileSummary.ThumbnailTableOffset))
	{
		return false;
...
bool FOasisPackageReader::ReadDependencyData(FOasisAssetData& OutAssetData)
{
	...
	const bool bReadPackageData = false;
	if (bReadPackageData/*EnumHasAnyFlags(Options, EReadOptions::PackageData)*/)
	{
		DependencyData.bHasPackageData = true;
		FAssetPackageData& PackageData = DependencyData.PackageData;
		...
		if (!SerializeImportedClasses(DependencyData.ImportMap, PackageData.ImportedClasses))
		{
			return false;
		}
	...
	const bool bReadDependencies = true;
	if (bReadDependencies/*EnumHasAnyFlags(Options, EReadOptions::Dependencies)*/)
	{
		DependencyData.bHasDependencyData = true;
		...
		if (!SerializeAssetRegistryDependencyData(DependencyData))
		{
			return false;
		}
...
bool FOasisPackageReader::ReadThumbnail(FObjectThumbnail& OutThumbnail)
{
...
bool FOasisPackageReader::ReadAssetRegistryDataIfCookedPackage(TArray<FAssetData*>& AssetDataList, TArray<FString>& CookedPackageNamesWithoutAssetData)
{
...
bool FOasisPackageReader::ReadDependencyData(FExtPackageDependencyData& OutDependencyData)
{
...
bool FOasisPackageReader::SerializeNameMap()
{
...
bool FOasisPackageReader::SerializeImportMap()
{
	if (ImportMap.Num() > 0)
	{
		return true;
...
bool FOasisPackageReader::SerializeImportedClasses(const TArray<FObjectImport>& InImportMap, TArray<FName>& OutClassNames)
{
	OutClassNames.Reset();
...
bool FOasisPackageReader::SerializeExportMap()
{
	if (ExportMap.Num() > 0)
	{
		return true;
...
bool FOasisPackageReader::SerializeSoftPackageReferenceList(TArray<FName>& OutSoftPackageReferenceList)
{
...
bool FOasisPackageReader::SerializeSearchableNamesMap(FExtPackageDependencyData& OutDependencyData)
{
...
void FOasisPackageReader::Serialize( void* V, int64 Length )
{
	check(Loader);
	Loader->Serialize( V, Length );
}

bool FOasisPackageReader::Precache( int64 PrecacheOffset, int64 PrecacheSize )
{
	check(Loader);
	return Loader->Precache( PrecacheOffset, PrecacheSize );
}

void FOasisPackageReader::Seek( int64 InPos )
{
	check(Loader);
	Loader->Seek( InPos );
}

int64 FOasisPackageReader::Tell()
{
	check(Loader);
	return Loader->Tell();
}

int64 FOasisPackageReader::TotalSize()
{
	check(Loader);
	return Loader->TotalSize();
}

uint32 FOasisPackageReader::GetPackageFlags() const
{
	return PackageFileSummary.GetPackageFlags();
}

const FPackageFileSummary& FOasisPackageReader::GetPackageFileSummary() const
{
	return PackageFileSummary;
}

int32 FOasisPackageReader::GetSoftPackageReferencesCount() const
{
	if (UEVer() >= VER_UE4_ADD_STRING_ASSET_REFERENCES_MAP && PackageFileSummary.SoftPackageReferencesOffset > 0 && PackageFileSummary.SoftPackageReferencesCount > 0)
	{
...
FArchive& FOasisPackageReader::operator<<( FName& Name )
{
	check(Loader);
