 ...
 */
class FOasisPackageReader : public FArchiveUObject
{
public:
	FOasisPackageReader();
	~FOasisPackageReader();
	...
	bool SerializeAssetRegistryDependencyData(FExtPackageDependencyData& DependencyData);
	...
	/** Reads information from the asset registry data table and converts it to FAssetData */
	bool ReadAssetRegistryData(FOasisAssetData& OutAssetData);
	/** Attempts to get the class name of an object from the thumbnail cache for packages older than VER_UE4_ASSET_REGISTRY_TAGS */
	bool ReadAssetDataFromThumbnailCache(FOasisAssetData& OutAssetData);
	/** Reads information used by the dependency graph */
	bool ReadDependencyData(FOasisAssetData& OutAssetData);
