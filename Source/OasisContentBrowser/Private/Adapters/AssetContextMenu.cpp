...
#include "OasisContentBrowserMenuContexts.h"
...
FAssetContextMenu::FAssetContextMenu(const TWeakPtr<SOasisAssetView>& InAssetView)
...
TSharedRef<SWidget> FAssetContextMenu::MakeContextMenu(const TArray<FOasisAssetData>& InSelectedAssets, const FSourcesData& InSourcesData, TSharedPtr< FUICommandList > InCommandList)
{
	...
	UOasisContentBrowserAssetContextMenuContext* ContextObject = NewObject<UOasisContentBrowserAssetContextMenuContext>();
	ContextObject->AssetContextMenu = SharedThis(this);

	UToolMenus* ToolMenus = UToolMenus::Get();

	static const FName BaseMenuName("OasisContentBrowser.AssetContextMenu");
...
void FAssetContextMenu::RegisterContextMenu(const FName MenuName)
{
	...
	if (!ToolMenus->IsMenuRegistered(MenuName))
	{
		...
		{
			UOasisContentBrowserAssetContextMenuContext* Context = InSection.FindContext<UOasisContentBrowserAssetContextMenuContext>();
		...
		{
			UOasisContentBrowserAssetContextMenuContext* Context = InMenu->FindContext<UOasisContentBrowserAssetContextMenuContext>();
			if (Context && Context->CommonAssetTypeActions.IsValid())
			{
...
void FAssetContextMenu::SetSelectedAssets(const TArray<FOasisAssetData>& InSelectedAssets)
{
...
void FAssetContextMenu::ExportSelectedAssetsToText()
{
	...
	for (const FOasisAssetData& Asset : SelectedAssets)
	{
...
void FAssetContextMenu::ExecuteFindInAssetTree(TArray<FName> InAssets)
{
	...
	TArray<FOasisAssetData> FoundLocalizedAssetData;
	FOasisContentBrowserSingleton::GetAssetRegistry().GetAssets(ARFilter, FoundLocalizedAssetData);
...
bool FAssetContextMenu::AddAssetTypeMenuOptions(UToolMenu* Menu, bool bHasObjectsSelected)
{
	...
	if (bHasObjectsSelected)
	{
		...
		UOasisContentBrowserAssetContextMenuContext* Context = Menu->FindContext<UOasisContentBrowserAssetContextMenuContext>();
		if (Context)
		{
			FToolMenuSection& Section = Menu->FindOrAddSection("GetAssetActions");
...
void FAssetContextMenu::ExecuteSyncToAssetTree()
{
	...
	const TArray<FOasisAssetData> SelectedAssetsCopy = SelectedAssets;
...
{
	OasisContentBrowserUtils::CopyFilePathsToClipboard(SelectedAssets);
}
...
void FAssetContextMenu::ExecuteFindInExplorer()
{
	for (int32 AssetIdx = 0; AssetIdx < SelectedAssets.Num(); ++AssetIdx)
	{
		...
		if (Asset)
		{
			FOasisAssetData AssetData(Asset);
...
#endif
		FOasisAssetData& AssetData = SelectedAssets[AssetIdx];
...
void FAssetContextMenu::ExecuteReparseAsset()
{
	if (SelectedAssets.Num() > 0 )
	{
		...
		for (FOasisAssetData& AssetData : SelectedAssets)
		{
			SlowTask.EnterProgressFrame();
			if (FOasisAssetData* CachedAssetData = AssetRegistry.GetCachedAssetByFilePath(AssetData.PackageFilePath))
			{
...
void FAssetContextMenu::ExecuteSyncToContentBrowser()
{
	if (SelectedAssets.Num() == 1)
	{
		FOasisAssetData& AssetData = SelectedAssets[0];
		if (AssetData.IsValid())
		{
			const bool bFound = FOasisAssetDataUtil::SyncToContentBrowser(AssetData);
			if (!bFound)
			{
				OasisContentBrowserUtils::NotifyMessage(FText::FromString(TEXT("No corresponding asset found in Content Browser.")));
			}
		}
	}
}
...
{
#if ECB_LEGACY
	TArray< FOasisAssetData > AssetViewSelectedAssets = AssetView.Pin()->GetSelectedAssets();
...
void FAssetContextMenu::ExecuteReload()
{
	...
	if (AssetViewSelectedAssets.Num() > 0)
	{
		...
		{
			const FOasisAssetData& AssetData = *AssetIt;
