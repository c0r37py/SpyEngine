...
bool DragDropHandler::ValidateDragDropOnAssetFolder(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent, const FString& TargetPath, bool& OutIsKnownDragOperation)
{
	...
	const bool bIsAssetPath = !OasisContentBrowserUtils::IsClassPath(TargetPath);
	...
	if (Operation->IsOfType<FAssetDragDropOp>())
	{
		...
		OasisContentBrowserUtils::CountItemTypes(DroppedAssets, NumAssetItems, NumClassItems);

		int32 NumAssetPaths, NumClassPaths;
		OasisContentBrowserUtils::CountPathTypes(DroppedAssetPaths, NumAssetPaths, NumClassPaths);
...
void DragDropHandler::HandleDropOnAssetFolder(const TSharedRef<SWidget>& ParentWidget, const TArray<FAssetData>& AssetList, const TArray<FString>& AssetPaths, const FString& TargetPath, const FText& TargetDisplayName, FExecuteCopyOrMove CopyActionHandler, FExecuteCopyOrMove MoveActionHandler, FExecuteCopyOrMove AdvancedCopyActionHandler)
{
	...
	{
		return OasisContentBrowserUtils::IsClassPath(AssetPath);
	});
...
void DragDropHandler::HandleDropOnAssetFolderOfContentBrowser(const TSharedRef<SWidget>& ParentWidget, const TArray<FOasisAssetData>& AssetList, const TArray<FString>& AssetPaths, const FString& TargetPath, const FText& TargetDisplayName, FExecuteDropToImport ImportActionHandler, FExecuteDropToImport ImportFlatActionHandler)
{
	// Remove any classes from the asset list
	TArray<FOasisAssetData> FinalAssetList = AssetList;
	FinalAssetList.RemoveAll([](const FOasisAssetData& AssetData)
	{
