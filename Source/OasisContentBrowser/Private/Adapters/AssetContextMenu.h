...
public:
	/** Constructor */
	FAssetContextMenu(const TWeakPtr<SOasisAssetView>& InAssetView);
	...
	TSharedRef<SWidget> MakeContextMenu(const TArray<FOasisAssetData>& SelectedAssets, const FSourcesData& InSourcesData, TSharedPtr< FUICommandList > InCommandList);

	/** Updates the list of currently selected assets to those passed in */
	void SetSelectedAssets(const TArray<FOasisAssetData>& InSelectedAssets);
...
private:
	...
	TWeakPtr<SOasisAssetView> AssetView;
