...
//-------------------------------------------------------------------------------------
// SOasisAssetThumbnail - Thumbnail widget for display thumbnail of .uasset file
//

class SOasisAssetThumbnail : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SOasisAssetThumbnail )
		...
		SLATE_ARGUMENT(TSharedPtr<FOasisAssetThumbnail>, AssetThumbnail)
		SLATE_ARGUMENT(TSharedPtr<FOasisAssetThumbnailPool>, ThumbnailPool)
	...
	void Construct( const FArguments& InArgs )
	{
		...
		AssetThumbnail->OnAssetDataChanged().AddSP(this, &SOasisAssetThumbnail::OnAssetDataChanged);

		const FOasisAssetData& AssetData = AssetThumbnail->GetAssetData();
		...
		[
			...
			.Visibility(this, &SOasisAssetThumbnail::GetGenericThumbnailVisibility)
			[
				SNew(SOverlay)
				...
				[
					...
					.DesiredSizeOverride(this, &SOasisAssetThumbnail::GetGenericThumbnailDesiredSize)
					.Image(this, &SOasisAssetThumbnail::GetClassThumbnailBrush)
				]
			]
		];
		...
		if ( InArgs._ThumbnailPool.IsValid() && !InArgs._ForceGenericThumbnail )
		{
			...
			InArgs._ThumbnailPool->OnThumbnailRendered().AddSP(this, &SOasisAssetThumbnail::OnThumbnailRendered);
			InArgs._ThumbnailPool->OnThumbnailRenderFailed().AddSP(this, &SOasisAssetThumbnail::OnThumbnailRenderFailed);
			...
			[
				...
				.ColorAndOpacity(this, &SOasisAssetThumbnail::GetViewportColorAndOpacity)
				.HAlign(HAlign_Center)
				.VAlign(VAlign_Center)
				[
		...
		if( ThumbnailClass.Get() && bIsClassType)
		{
			...
			.HAlign(HAlign_Right)
			.Padding(TAttribute<FMargin>(this, &SOasisAssetThumbnail::GetClassIconPadding))
			[
				SAssignNew(ClassIconWidget, SBorder)
				.BorderImage(FAppStyle::GetNoBrush())
				[
					SNew(SImage)
					.Image(this, &SOasisAssetThumbnail::GetClassIconBrush)
				]
			];
		}
		...
		if( bAllowHintText )
		{
			OverlayWidget->AddSlot()
				...
				[
					...
					.BorderBackgroundColor(this, &SOasisAssetThumbnail::GetHintBackgroundColor) //Adjust the opacity of the border itself
					.ColorAndOpacity(HintColorAndOpacity) //adjusts the opacity of the contents of the border
					.Visibility(this, &SOasisAssetThumbnail::GetHintTextVisibility)
					.Padding(0)
					[
		...
		[
			...
			.Padding(this, &SOasisAssetThumbnail::GetAssetColorStripPadding)
		];
...
#if ECB_LEGACY
		if (!GetDefault<UOasisContentBrowserSettings>()->RealTimeThumbnails )
		{
...
private:
	...
	void OnThumbnailRendered(const FOasisAssetData& AssetData)
	{
	...
	void OnThumbnailRenderFailed(const FOasisAssetData& AssetData)
	{
	...
	FText GetDisplayNameForClass( UClass* Class, const FOasisAssetData* InOasisAssetData = nullptr) const
	{
		...
		if ( Class )
		{
			...
			if ( AssetTypeActions.IsValid() )
			{
				if (InOasisAssetData != nullptr)
				{
					...
					{
						ClassDisplayName = BaseAssetTypeAction->GetDisplayNameFromAssetData(InOasisAssetData->AssetData);
					}
				}
	...
	{
		const FOasisAssetData& OasisAssetData = AssetThumbnail->GetAssetData();

		if ( OasisAssetData.GetClass() == UClass::StaticClass() )
		{
			UClass* Class = Cast<UClass>( OasisAssetData.GetAsset() );
			return GetDisplayNameForClass( Class );
		}

		return FText::FromName(OasisAssetData.AssetName);
	}
...
//-------------------------------------------------------------------------------------
// FOasisAssetThumbnail implementation
//

FOasisAssetThumbnail::FOasisAssetThumbnail( UObject* InAsset, uint32 InWidth, uint32 InHeight, const TSharedPtr<class FOasisAssetThumbnailPool>& InThumbnailPool )
	: ThumbnailPool(InThumbnailPool)
	, AssetData(InAsset ? FOasisAssetData(InAsset) : FOasisAssetData())
	, Width( InWidth )
	, Height( InHeight )
{
...
FOasisAssetThumbnail::FOasisAssetThumbnail( const FOasisAssetData& InAssetData , uint32 InWidth, uint32 InHeight, const TSharedPtr<class FOasisAssetThumbnailPool>& InThumbnailPool )
	: ThumbnailPool( InThumbnailPool )
...
FOasisAssetThumbnail::~FOasisAssetThumbnail()
{
...
FIntPoint FOasisAssetThumbnail::GetSize() const
{
	return FIntPoint( Width, Height );
}

FSlateShaderResource* FOasisAssetThumbnail::GetViewportRenderTargetTexture() const
{
...
UObject* FOasisAssetThumbnail::GetAsset() const
{
...
const FOasisAssetData& FOasisAssetThumbnail::GetAssetData() const
{
	return AssetData;
}

void FOasisAssetThumbnail::SetAsset( const UObject* InAsset )
{
	SetAsset( FOasisAssetData(InAsset) );
}

void FOasisAssetThumbnail::SetAsset( const FOasisAssetData& InAssetData )
{
	if ( ThumbnailPool.IsValid() )
	{
		ThumbnailPool.Pin()->RemoveReferencer(*this);
...
// 	else
// 	{
// 		AssetData = FOasisAssetData();
// 	}

	AssetDataChangedEvent.Broadcast();
}

TSharedRef<SWidget> FOasisAssetThumbnail::MakeThumbnailWidget( const FOasisAssetThumbnailConfig& InConfig )
{
	return
		SNew(SOasisAssetThumbnail)
...
void FOasisAssetThumbnail::RefreshThumbnail()
{
...
TSharedPtr<SWidget> FOasisAssetThumbnail::GetSpecificThumbnailOverlay() const
{
	TSharedRef<SOverlay> ThumbnailOverlayWidget = SNew(SOverlay);

	const bool bDisplayEngineVersionOverlay = GetDefault<UOasisContentBrowserSettings>()->DisplayEngineVersionOverlay;
	if (bDisplayEngineVersionOverlay)
	{
		...
		[
			SNew(SBorder)
			.BorderImage(FAppStyle::GetNoBrush())
			//.Visibility(TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateStatic(&FOasisAssetThumbnailOverlayManager::GetSpecificThumbnailOverlayVisibility, InAssetData.AssetClass)))
			...
			[
				...
				.TextStyle(FOasisContentBrowserStyle::Get(), "SpyEngine.AssetThumbnail.EngineOverlay")
				.Justification(ETextJustify::Center)
			]
		];
	}
	...
	const bool bDisplayValidationOverlay = GetDefault<UOasisContentBrowserSettings>()->DisplayValidationStatusOverlay;
	if (bDisplayValidationOverlay)
	{
		...
		[
			...
			[
				SNew(SImage)
				.Image(this, &FOasisAssetThumbnail::GetValidationIconBrush)
			]
		];
	}
#endif
	...
	const bool bDisplayContentTypeOverlay = GetDefault<UOasisContentBrowserSettings>()->DisplayContentTypeOverlay;
	if (bDisplayContentTypeOverlay)
	{
		ThumbnailOverlayWidget->AddSlot()
			[
				SNew(SBorder)
				.BorderImage(FAppStyle::GetNoBrush())
				.Visibility(this, &FOasisAssetThumbnail::GetContentTypeOverlayVisibility)
				.Padding(FMargin(1.0f, 1.0f, 1.0f, 1.0f))
				.HAlign(HAlign_Right)
				.VAlign(VAlign_Top)
				[
...
const FSlateBrush* FOasisAssetThumbnail::GetValidationIconBrush() const
{
#if ECB_FEA_VALIDATE_OVERLAY

	if (!AssetData.IsValid())
	{
		return FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.ValidationInValid");
	}

	if (const FExtAssetDependencyInfo* DependencyInfoPtr = FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetDependencyInfo(AssetData))
	{
		...
		{
			return FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.ValidationValid");
		}
		else if (DependencyInfo.AssetStatus == EDependencyNodeStatus::ValidWithSoftReferenceIssue)
		{
			return FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.ValidationIssue");
		}
		else
		{
			return FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.ValidationUknown");
		}
	}

	return FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.ValidationUknown");

#else  // ECB_FEA_VALIDATE_OVERLAY

	return nullptr;

#endif // ECB_FEA_VALIDATE_OVERLAY
}

EVisibility FOasisAssetThumbnail::GetContentTypeOverlayVisibility() const
{
	//return EVisibility::Collapsed;
	return EVisibility::Visible;
}
...
//-------------------------------------------------------------------------------------
// FOasisAssetThumbnailPool implementation
//

FOasisAssetThumbnailPool::FOasisAssetThumbnailPool( uint32 InNumInPool, const TAttribute<bool>& InAreRealTimeThumbnailsAllowed, double InMaxFrameTimeAllowance, uint32 InMaxRealTimeThumbnailsPerFrame )
...
{
	FCoreUObjectDelegates::OnObjectPropertyChanged.AddRaw(this, &FOasisAssetThumbnailPool::OnObjectPropertyChanged);
	FCoreUObjectDelegates::OnAssetLoaded.AddRaw(this, &FOasisAssetThumbnailPool::OnAssetLoaded);
	if ( GEditor )
	{
		GEditor->OnActorMoved().AddRaw( this, &FOasisAssetThumbnailPool::OnActorPostEditMove );
	}
}

FOasisAssetThumbnailPool::~FOasisAssetThumbnailPool()
{
...
FOasisAssetThumbnailPool::FThumbnailInfo::~FThumbnailInfo()
{
	if( ThumbnailTexture )
	{
...
void FOasisAssetThumbnailPool::ReleaseResources()
{
...
TStatId FOasisAssetThumbnailPool::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT( FOasisAssetThumbnailPool, STATGROUP_Tickables );
}

bool FOasisAssetThumbnailPool::IsTickable() const
{
...
void FOasisAssetThumbnailPool::Tick( float DeltaTime )
{
...
#if ECB_LEGACY
	const bool bShouldUseRealtimeThumbnails = AreRealTimeThumbnailsAllowed.Get() && GetDefault<UOasisContentBrowserSettings>()->RealTimeThumbnails && !bIsInPIEOrSimulate;
#else
			...
			if( Info.IsValid() )
			{
				...
				if (InfoRef->AssetData.HasThumbnail())
				{
					...
					if ( !bLoadedThumbnail )
					{
						...
						const bool bUseThumbnailPool = GetDefault<UOasisContentBrowserSettings>()->bUseThumbnailPool;

#if ECB_WIP_THUMB_CACHE
						const FObjectThumbnail* FoundThumbnail = FOasisContentBrowserSingleton::GetAssetRegistry().FindCachedThumbnail(InfoRef->AssetData.PackageFilePath);
#else

	#if ECB_WIP_OBJECT_THUMB_POOL
						const FObjectThumbnail* FoundThumbnail = bUseThumbnailPool 
							? FOasisContentBrowserSingleton::GetThumbnailPool().FindCachedThumbnail(InfoRef->AssetData.GetFullName())
						...
						else
						{
							...
							FObjectThumbnail& ObjectThumbnailRef = bUseThumbnailPool 
								? FOasisContentBrowserSingleton::GetThumbnailPool().AddToPool(InfoRef->AssetData.GetFullName())
								: ObjectThumbnail;
...
FSlateTexture2DRHIRef* FOasisAssetThumbnailPool::AccessTexture( const FOasisAssetData& AssetData, uint32 Width, uint32 Height )
{
...
void FOasisAssetThumbnailPool::AddReferencer( const FOasisAssetThumbnail& AssetThumbnail )
{
...
void FOasisAssetThumbnailPool::RemoveReferencer( const FOasisAssetThumbnail& AssetThumbnail )
{
	FIntPoint Size = AssetThumbnail.GetSize();
	...
	{
		// This FOasisAssetThumbnail did not reference anything or was deleted after the pool was deleted.
	}
}

bool FOasisAssetThumbnailPool::IsInRenderStack( const TSharedPtr<FOasisAssetThumbnail>& Thumbnail ) const
{
	const FOasisAssetData& AssetData = Thumbnail->GetAssetData();
...
bool FOasisAssetThumbnailPool::IsRendered(const TSharedPtr<FOasisAssetThumbnail>& Thumbnail) const
{
	const FOasisAssetData& AssetData = Thumbnail->GetAssetData();
	const uint32 Width = Thumbnail->GetSize().X;
	const uint32 Height = Thumbnail->GetSize().Y;
...
void FOasisAssetThumbnailPool::PrioritizeThumbnails( const TArray< TSharedPtr<FOasisAssetThumbnail> >& ThumbnailsToPrioritize, uint32 Width, uint32 Height )
{
...
void FOasisAssetThumbnailPool::RefreshThumbnail( const TSharedPtr<FOasisAssetThumbnail>& ThumbnailToRefresh )
{
	const FOasisAssetData& AssetData = ThumbnailToRefresh->GetAssetData();
...
void FOasisAssetThumbnailPool::FreeThumbnail( const FName& ObjectPath, uint32 Width, uint32 Height )
{
...
void FOasisAssetThumbnailPool::RefreshThumbnailsFor( FName ObjectPath )
{
	for ( auto ThumbIt = ThumbnailToTextureMap.CreateIterator(); ThumbIt; ++ThumbIt)
	{
...
void FOasisAssetThumbnailPool::OnAssetLoaded( UObject* Asset )
{
	if ( Asset != NULL )
	{
...
void FOasisAssetThumbnailPool::OnActorPostEditMove( AActor* Actor )
{
	DirtyThumbnailForObject(Actor);
}

void FOasisAssetThumbnailPool::OnObjectPropertyChanged( UObject* ObjectBeingModified, FPropertyChangedEvent& PropertyChangedEvent )
{
	DirtyThumbnailForObject(ObjectBeingModified);
}

void FOasisAssetThumbnailPool::DirtyThumbnailForObject(UObject* ObjectBeingModified)
{
