...
FOasisFeedbackContextEditor::FOasisFeedbackContextEditor()
	: HasTaskBeenCancelled(false)
{
	
}

void FOasisFeedbackContextEditor::Serialize( const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category )
{
...
void FOasisFeedbackContextEditor::StartSlowTask( const FText& Task, bool bShowCancelButton )
{
		...
		if (ParentWindow.IsValid())
		{
			...
			if (!bProgressWindowShown && !bHaveOpenMenu && FSlateApplication::Get().CanDisplayWindows())
			{
				...
				if (bShowCancelButton)
				{
					...
					OnCancelClicked = FOnCancelClickedDelegate::CreateRaw(this, &FOasisFeedbackContextEditor::OnUserCancel);
				}
...
void FOasisFeedbackContextEditor::FinalizeSlowTask()
{
...
void FOasisFeedbackContextEditor::ProgressReported( const float TotalProgressInterp, FText DisplayMessage )
{
...
bool FOasisFeedbackContextEditor::ReceivedUserCancel( void )
{
	return HasTaskBeenCancelled;
}

void FOasisFeedbackContextEditor::OnUserCancel()
{
 ...
 */
TWeakPtr<class SBuildProgressWidget> FOasisFeedbackContextEditor::ShowBuildProgressWindow()
{
	TSharedRef<SWindow> BuildProgressWindowRef = SNew(SWindow)
		.ClientSize(FVector2D(500,200))
		.IsPopupWindow(true);

	BuildProgressWidget = SNew(SOasisBuildProgressWidget);
...
void FOasisFeedbackContextEditor::CloseBuildProgressWindow()
{
...
bool FOasisFeedbackContextEditor::IsPlayingInEditor() const
{
