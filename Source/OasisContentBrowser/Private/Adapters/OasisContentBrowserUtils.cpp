...
namespace OasisContentBrowserUtils
{
...
class SOasisContentBrowserPopup : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SOasisContentBrowserPopup ){}
	...
	void Construct( const FArguments& InArgs )
	{
		...
		[
			...
			.Padding(10)
			.OnMouseButtonDown(this, &SOasisContentBrowserPopup::OnBorderClicked)
			.BorderBackgroundColor(this, &SOasisContentBrowserPopup::GetBorderBackgroundColor)
			[
				SNew(SHorizontalBox)
	...
	{
		TSharedRef<SOasisContentBrowserPopup> PopupContent = SNew(SOasisContentBrowserPopup) .Message(Message);
...
class SOasisContentBrowserConfirmPopup :  public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SOasisContentBrowserConfirmPopup ) {}
			...
			[
				...
				[
					...
					[
						...
						.OnClicked( this, &SOasisContentBrowserConfirmPopup::YesClicked )
					]
					...
					[
						...
						.HAlign(HAlign_Center)
						.Text(InArgs._NoText)
						.OnClicked( this, &SOasisContentBrowserConfirmPopup::NoClicked )
					]
				]
			]
		];
...
bool OasisContentBrowserUtils::OpenEditorForAsset(const FString& ObjectPath)
{
	// Load the asset if unloaded
	TArray<UObject*> LoadedObjects;
	TArray<FString> ObjectPaths;
	ObjectPaths.Add(ObjectPath);
	OasisContentBrowserUtils::LoadAssetsIfNeeded(ObjectPaths, LoadedObjects);
...
bool OasisContentBrowserUtils::OpenEditorForAsset(UObject* Asset)
{
	if( Asset != NULL )
	{
...
bool OasisContentBrowserUtils::OpenEditorForAsset(const TArray<UObject*>& Assets)
{
	if ( Assets.Num() == 1 )
	{
...
bool OasisContentBrowserUtils::LoadAssetsIfNeeded(const TArray<FString>& ObjectPaths, TArray<UObject*>& LoadedObjects, bool bAllowedToPromptToLoadAssets, bool bLoadRedirects)
{
	...
	if ( UnloadedObjectPaths.Num() > 0 )
	{
		...
		const bool bShowProgressDialog = (UnloadedObjectPaths.Num() > GetDefault<UOasisContentBrowserSettings>()->NumObjectsToLoadBeforeWarning) || bAtLeastOneUnloadedMap;
...
void OasisContentBrowserUtils::GetUnloadedAssets(const TArray<FString>& ObjectPaths, TArray<FString>& OutUnloadedObjects)
{
...
bool OasisContentBrowserUtils::PromptToLoadAssets(const TArray<FString>& UnloadedObjects)
{
...
bool OasisContentBrowserUtils::CanRenameFolder(const FString& InFolderPath)
{
	// Cannot rename folders that are part of a classes or collections root
	return !OasisContentBrowserUtils::IsClassPath(InFolderPath) && !OasisContentBrowserUtils::IsCollectionPath(InFolderPath);
}

bool OasisContentBrowserUtils::CanRenameAsset(const FAssetData& InAssetData)
{
...
void OasisContentBrowserUtils::RenameAsset(UObject* Asset, const FString& NewName, FText& ErrorMessage)
{
...
void OasisContentBrowserUtils::CopyAssets(const TArray<UObject*>& Assets, const FString& DestPath)
{
#if ECB_LEGACY
...
void OasisContentBrowserUtils::MoveAssets(const TArray<UObject*>& Assets, const FString& DestPath, const FString& SourcePath)
{
...
int32 OasisContentBrowserUtils::DeleteAssets(const TArray<UObject*>& AssetsToDelete)
{
	return ObjectTools::DeleteObjects(AssetsToDelete);
}

bool OasisContentBrowserUtils::DeleteFolders(const TArray<FString>& PathsToDelete)
{
	...
	else
	{
		...
		if ( OasisContentBrowserUtils::LoadAssetsIfNeeded(ObjectPaths, LoadedAssets) )
		{
			...
			if ( LoadedAssets.Num() == NumAssetsInPaths )
			{
				...
				const int32 NumAssetsDeleted = OasisContentBrowserUtils::DeleteAssets(ToDelete);
...
bool OasisContentBrowserUtils::DeleteEmptyFolderFromDisk(const FString& InPathToDelete)
{
...
void OasisContentBrowserUtils::GetAssetsInPaths(const TArray<FString>& InPaths, TArray<FOasisAssetData>& OutAssetDataList)
{
	...
	FOasisContentBrowserSingleton::GetAssetRegistry().GetAssets(Filter, OutAssetDataList);
}

bool OasisContentBrowserUtils::SavePackages(const TArray<UPackage*>& Packages)
{
...
bool OasisContentBrowserUtils::SaveDirtyPackages()
{
...
TArray<UPackage*> OasisContentBrowserUtils::LoadPackages(const TArray<FString>& PackageNames)
{
...
void OasisContentBrowserUtils::NotifyMessage(const FText& Message, bool bAlsoPrintToConsole, float InDuration /*= 1.0f*/)
{
	if (!Message.IsEmpty())
	{
...
void OasisContentBrowserUtils::NotifyMessage(const FString& Message, float InDuration /*= 3.f*/)
{
	NotifyMessage(FText::FromString(Message), false, InDuration);
}

void OasisContentBrowserUtils::DisplayMessage(const FText& Message, const FSlateRect& ScreenAnchor, const TSharedRef<SWidget>& ParentContent)
{
	SOasisContentBrowserPopup::DisplayMessage(Message, ScreenAnchor, ParentContent);
}

void OasisContentBrowserUtils::DisplayMessagePopup(const FText& Message)
{
	FMessageDialog::Open(EAppMsgType::Ok, Message);
}

void OasisContentBrowserUtils::DisplayConfirmationPopup(const FText& Message, const FText& YesString, const FText& NoString, const TSharedRef<SWidget>& ParentContent, const FOnClicked& OnYesClicked, const FOnClicked& OnNoClicked)
{
	TSharedRef<SOasisContentBrowserConfirmPopup> Popup = 
		SNew(SOasisContentBrowserConfirmPopup)
		.Prompt(Message)
		.YesText(YesString)
		.NoText(NoString)
...
bool OasisContentBrowserUtils::RenameFolder(const FString& DestPath, const FString& SourcePath)
{
...
#endif
bool OasisContentBrowserUtils::CopyFolders(const TArray<FString>& InSourcePathNames, const FString& DestPath)
{
...
#if ECB_LEGACY
bool OasisContentBrowserUtils::MoveFolders(const TArray<FString>& InSourcePathNames, const FString& DestPath)
{
	TMap<FString, TArray<UObject*> > SourcePathToLoadedAssets;
...
bool OasisContentBrowserUtils::PrepareFoldersForDragDrop(const TArray<FString>& SourcePathNames, TMap< FString, TArray<UObject*> >& OutSourcePathToLoadedAssets)
{
...
void OasisContentBrowserUtils::CopyAssetReferencesToClipboard(const TArray<FAssetData>& AssetsToCopy)
{
...
void OasisContentBrowserUtils::CopyFilePathsToClipboard(const TArray<FOasisAssetData>& AssetsToCopy)
{
	FString ClipboardText;
	for (const FOasisAssetData& Asset : AssetsToCopy)
	{
		if (ClipboardText.Len() > 0)
		{
			ClipboardText += LINE_TERMINATOR;
...
void OasisContentBrowserUtils::CaptureThumbnailFromViewport(FViewport* InViewport, const TArray<FAssetData>& InAssetsToAssign)
{
...
void OasisContentBrowserUtils::ClearCustomThumbnails(const TArray<FAssetData>& InAssetsToAssign)
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
...
bool OasisContentBrowserUtils::AssetHasCustomThumbnail( const FAssetData& AssetData )
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
...
OasisContentBrowserUtils::ECBFolderCategory OasisContentBrowserUtils::GetFolderCategory( const FString& InPath )
{
...
OasisContentBrowserUtils::ECBFolderCategory OasisContentBrowserUtils::GetRootFolderCategory(const FString& InPath)
{
	FText LocalizedFolderName = FText::GetEmpty();
	FOasisAssetData::EContentType ContentType;
	if (FOasisContentBrowserSingleton::GetAssetRegistry().QueryRootContentPathInfo(InPath, &LocalizedFolderName, &ContentType))
	{
		if (ContentType == FOasisAssetData::EContentType::Plugin)
		{
			return ECBFolderCategory::PluginContent;
		}
		if (ContentType == FOasisAssetData::EContentType::Project)
		{
			return ECBFolderCategory::ProjectContent;
		}
		if (ContentType == FOasisAssetData::EContentType::VaultCache)
		{
...
bool OasisContentBrowserUtils::IsEngineFolder( const FString& InPath )
{
...
bool OasisContentBrowserUtils::IsDevelopersFolder( const FString& InPath )
{
...
bool OasisContentBrowserUtils::IsPluginFolder(const FString& InPath, const TArray<TSharedRef<IPlugin>>& InPlugins, EPluginLoadedFrom* OutPluginSource)
{
...
bool OasisContentBrowserUtils::IsPluginFolder(const FString& InPath, EPluginLoadedFrom* OutPluginSource)
{
	return IsPluginFolder(InPath, IPluginManager::Get().GetEnabledPluginsWithContent(), OutPluginSource);
}

bool OasisContentBrowserUtils::IsClassesFolder(const FString& InPath)
{
...
bool OasisContentBrowserUtils::IsLocalizationFolder( const FString& InPath )
{
	return FPackageName::IsLocalizedPackage(InPath);
}

void OasisContentBrowserUtils::GetObjectsInAssetData(const TArray<FAssetData>& AssetList, TArray<UObject*>& OutDroppedObjects)
{	
...
bool OasisContentBrowserUtils::IsValidFolderName(const FString& FolderName, FText& Reason)
{
...
bool OasisContentBrowserUtils::DoesFolderExist(const FString& FolderPath)
{
...
bool OasisContentBrowserUtils::IsEmptyFolder(const FString& FolderPath, const bool bRecursive)
{
	if (OasisContentBrowserUtils::IsClassPath(FolderPath))
	{
...
bool OasisContentBrowserUtils::IsRootDir(const FString& FolderPath)
{
	return IsAssetRootDir(FolderPath) || IsClassRootDir(FolderPath);
}

bool OasisContentBrowserUtils::IsAssetRootDir(const FString& FolderPath)
{
...
bool OasisContentBrowserUtils::IsClassRootDir(const FString& FolderPath)
{
...
FText OasisContentBrowserUtils::GetRootDirDisplayName(const FString& FolderPath)
{
#if ECB_DISABLE
	...
	FText LocalizedFolderName = FText::GetEmpty();
	FOasisAssetData::EContentType ContentType;
	if (!FOasisContentBrowserSingleton::GetAssetRegistry().QueryRootContentPathInfo(FolderPath, &LocalizedFolderName, &ContentType))
	{
		// fallback
		FString CleanFolderPath = FolderPath;
...
#if ECB_LEGACY
	const bool bIsBackgroundGathering = FOasisContentBrowserSingleton::GetAssetRegistry().IsFolderBackgroundGathering(FolderPath);
...
bool OasisContentBrowserUtils::IsFolderBackgroundGathering(const FString& InFolder)
{
	const bool bIsBackgroundGathering = FOasisContentBrowserSingleton::GetAssetRegistry().IsFolderBackgroundGathering(InFolder);
	return bIsBackgroundGathering;
}

FName OasisContentBrowserUtils::GetCurrentGatheringFolder()
{
	return FOasisContentBrowserSingleton::GetAssetRegistry().GetCurrentGatheringFolder();
}

bool OasisContentBrowserUtils::IsClassPath(const FString& InPath)
{
	static const FString ClassesRootPrefix = TEXT("/Classes_");
	return InPath.StartsWith(ClassesRootPrefix);
}

bool OasisContentBrowserUtils::IsCollectionPath(const FString& InPath, FName* OutCollectionName, ECollectionShareType::Type* OutCollectionShareType)
{
...
void OasisContentBrowserUtils::CountPathTypes(const TArray<FString>& InPaths, int32& OutNumAssetPaths, int32& OutNumClassPaths)
{
	OutNumAssetPaths = 0;
	OutNumClassPaths = 0;
...
void OasisContentBrowserUtils::CountPathTypes(const TArray<FName>& InPaths, int32& OutNumAssetPaths, int32& OutNumClassPaths)
{
	OutNumAssetPaths = 0;
	OutNumClassPaths = 0;
...
void OasisContentBrowserUtils::CountItemTypes(const TArray<FAssetData>& InItems, int32& OutNumAssetItems, int32& OutNumClassItems)
{
	OutNumAssetItems = 0;
	OutNumClassItems = 0;
...
bool OasisContentBrowserUtils::IsValidPathToCreateNewClass(const FString& InPath)
{
...
bool OasisContentBrowserUtils::IsValidPathToCreateNewFolder(const FString& InPath)
{
...
const TSharedPtr<FLinearColor> OasisContentBrowserUtils::LoadColor(const FString& FolderPath, bool bNoCache)
{
	auto LoadColorInternal = [](const FString& InPath, bool bInNoCache) -> TSharedPtr<FLinearColor>
	{
		...
		if(FPaths::FileExists(GEditorPerProjectIni))
		{
			...
			{
				FLinearColor Color;
				if(Color.InitFromString(ColorStr) && !Color.Equals(OasisContentBrowserUtils::GetDefaultColor()))
				{
			...
			{
				return PathColors.Add(InPath, MakeShareable(new FLinearColor(OasisContentBrowserUtils::GetDefaultColor())));
			}
		}
		return nullptr;
	};
...
bool OasisContentBrowserUtils::LoadFolderColor(const FString& FolderPath, FLinearColor& OutFolderColor)
{
	return FOasisContentBrowserSingleton::GetAssetRegistry().GetFolderColor(FolderPath, OutFolderColor);
}

void OasisContentBrowserUtils::SaveColor(const FString& FolderPath, const TSharedPtr<FLinearColor>& FolderColor, bool bForceAdd)
{
	...
	const bool bRemove = !FolderColor.IsValid() || (!bForceAdd && FolderColor->Equals(OasisContentBrowserUtils::GetDefaultColor()));
...
bool OasisContentBrowserUtils::HasCustomColors( TArray< FLinearColor >* OutColors )
{
	...
	if (FPaths::FileExists(GEditorPerProjectIni))
	{
		...
		for( int32 SectionIndex = 0; SectionIndex < Section.Num(); SectionIndex++ )
		{
			...
			if ( EntryStr.Split( TEXT( "=" ), &PathStr, &ColorStr ) )
			{
				...
				if( CurrentColor.InitFromString( ColorStr ) && !CurrentColor.Equals( OasisContentBrowserUtils::GetDefaultColor() ) )
				{
					bHasCustom = true;
					if ( OutColors )
					{
...
FLinearColor OasisContentBrowserUtils::GetDefaultColor()
{
	// The default tint the folder should appear as
	return FLinearColor::Gray;
}

FText OasisContentBrowserUtils::GetExploreFolderText()
{
	FFormatNamedArguments Args;
...
bool OasisContentBrowserUtils::IsValidObjectPathForCreate(const FString& ObjectPath, FText& OutErrorMessage, bool bAllowExistingAsset)
{
...
bool OasisContentBrowserUtils::IsValidFolderPathForCreate(const FString& InFolderPath, const FString& NewFolderName, FText& OutErrorMessage)
{
	if (!OasisContentBrowserUtils::IsValidFolderName(NewFolderName, OutErrorMessage))
	{
		return false;
	}
	...
	if (OasisContentBrowserUtils::DoesFolderExist(NewFolderPath))
	{
	...
	if (!bDisplayL10N && OasisContentBrowserUtils::IsLocalizationFolder(NewFolderPath))
	{
...
int32 OasisContentBrowserUtils::GetPackageLengthForCooking(const FString& PackageName, bool IsInternalBuild)
{
	...
	const bool bIsPluginAsset = OasisContentBrowserUtils::IsPluginFolder(PackageName, &PluginLoadedFrom);
	const bool bIsEngineAsset = OasisContentBrowserUtils::IsEngineFolder(PackageName) || (bIsPluginAsset && PluginLoadedFrom == EPluginLoadedFrom::Engine);
...
bool OasisContentBrowserUtils::IsValidPackageForCooking(const FString& PackageName, FText& OutErrorMessage)
{
...
bool OasisContentBrowserUtils::IsFavoriteFolder(const FString& FolderPath)
{
	return FOasisContentBrowserSingleton::Get().FavoriteFolderPaths.Contains(FolderPath);
}

void OasisContentBrowserUtils::AddFavoriteFolder(const FString& FolderPath, bool bFlushConfig /*= true*/)
{
	FOasisContentBrowserSingleton::Get().FavoriteFolderPaths.AddUnique(FolderPath);
}

void OasisContentBrowserUtils::RemoveFavoriteFolder(const FString& FolderPath, bool bFlushConfig /*= true*/)
{
	TArray<FString> FoldersToRemove;
	FoldersToRemove.Add(FolderPath);
	
	// Find and remove any subfolders
	for (const FString& FavoritePath : FOasisContentBrowserSingleton::Get().FavoriteFolderPaths)
	{
	...
	{
		FOasisContentBrowserSingleton::Get().FavoriteFolderPaths.Remove(FolderToRemove);
	}
	if (bFlushConfig)
	{
...
const TArray<FString>& OasisContentBrowserUtils::GetFavoriteFolders()
{
	return FOasisContentBrowserSingleton::Get().FavoriteFolderPaths;
}

int32 OasisContentBrowserUtils::GetMaxCookPathLen()
{
...
void OasisContentBrowserUtils::BeginAdvancedCopyPackages(TArray<FAssetData>& AssetList, TArray<FString>& AssetPaths, FString& DestinationPath)
{
	TMap<FString, TArray<UObject*> > SourcePathToLoadedAssets;
