...
namespace FExtAssetCoreUtil
{
	bool FExtAssetFeedbackContext::ReceivedUserCancel()
	{
		...
		{
			bTaskWasCancelledCache = FOasisFeedbackContextEditor::ReceivedUserCancel();
		}
			...
			if (const FConfigSection* Sec = ConfigFile.Find(TEXT("PathColor")))
			{
				...
				for (FConfigSectionMap::TConstIterator SecIt(*Sec); SecIt; ++SecIt)
				{
					...
					if (Color.InitFromString(ColorString))
					{
						...
						{
							FString FolderPath;
							FString PackageRoot = FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(PackageName);
							if (FOasisAssetDataUtil::RemapGamePackageToFullFilePath(PackageRoot, PackageName, InAssetContentRoot, FolderPath, /*bPackageIsFolder*/true))
							{
...
/////////////////////////////////////////////////////////////
// FOasisAssetData implementation
//

FOasisAssetData::FOasisAssetData()
	: bParsed(false)
	, bValid(false)
	, bHasThumbnail(false)
	, bCompatible(false)
...
FOasisAssetData::FOasisAssetData(const FString& InPackageFilePath, bool bDelayParse/* = false*/)
...
FOasisAssetData::FOasisAssetData(const UObject* InAsset, bool bAllowBlueprintClass)
{
...
FOasisAssetData::FOasisAssetData(const FName& InPackageName)
	: InvalidReason(EInvalidReason::NotParsed)
{
	if (FOasisAssetData* CachedAssetData = FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetByPackageName(InPackageName))
	{
	...
	{
		FOasisAssetData OasisAssetData(AbsolutePackageFilePath);
		*this = OasisAssetData;
		return;
	}
...
bool FOasisAssetData::DoParse()
{
	...
	bValid = bValid && FOasisPackageUtils::ParsePackageWithPackageReader(*this);
	if (bValid)
	{	
		if (AssetCount < 1)
		{
	...
	//if (bValid)
	{
		...
		if (FOasisContentBrowserSingleton::GetAssetRegistry().ParseAssetContentRoot(FilePath, RelativePath, BaseDir, AssetContentType, &AllDependencies))
		{
...
bool FOasisAssetData::Parse()
{
	return DoParse();
}

bool FOasisAssetData::ReParse()
{
	if (PreParse())
	{
		return Parse();
...
bool FOasisAssetData::HasThumbnail() const
{
	return bHasThumbnail;
}

bool FOasisAssetData::HasContentRoot() const
{
	return AssetContentRoot != NAME_None && AssetRelativePath != NAME_None;
}

bool FOasisAssetData::HasValidThumbnail() const
{
...
bool FOasisAssetData::CanImportFast() const
{
	return IsValid() && IsCompatibleWithCurrentEngineVersion() && HasContentRoot();
}

bool FOasisAssetData::LoadThumbnail(FObjectThumbnail& OutThumbnail) const
{
	return FOasisPackageUtils::LoadThumbnailWithPackageReader(*this, OutThumbnail);
}

#if ECB_LEGACY
void FOasisAssetData::LoadSoftReferences(TArray<FName>& OutSoftReferences) const
{
	FOasisPackageUtils::LoadSoftReferences(*this, OutSoftReferences);
}
#endif

FString FOasisAssetData::GetSavedEngineVersionForDisplay() const
{
	return SavedByEngineVersion.ToString(EVersionComponent::Patch);
}

const FString& FOasisAssetData::GetAssetContentTypeForDisplay() const
{
...
UClass* FOasisAssetData::GetIconClass(bool* bOutIsClassType) const
{
	if (bOutIsClassType)
	{
...
FSoftObjectPath FOasisAssetData::GetSoftObjectPath() const
{
	if (IsTopLevelAsset())
	{
...
FString FOasisAssetData::GetFolderPath() const
{
	return FPaths::GetPath(PackageFilePath.ToString());
}

FString FOasisAssetData::ConvertFilePathToTempPackageName(const FString& InFilePath)
{
...
FString& FOasisAssetData::GetSandboxPackagePath()
{
	static FString SandboxPackagePath = FExtAssetContants::ExtSandboxPackagePath; // TEXT("/SpyEngine/Sandbox");
	return SandboxPackagePath;
}

FString& FOasisAssetData::GetSandboxPackagePathWithSlash()
{
...
FString& FOasisAssetData::GetSandboxDir()
{
	const FString ProjectIntermediateDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectIntermediateDir());
	static FString SandboxDir = FPaths::Combine(ProjectIntermediateDir, TEXT("SpyEngine"), TEXT("Sandbox"));
	return SandboxDir;
}

FString& FOasisAssetData::GetSpyEngineTempDir()
{
	const FString ProjectIntermediateDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectIntermediateDir());
	static FString TempDir = FPaths::Combine(ProjectIntermediateDir, TEXT("SpyEngine"), TEXT("Temp"));
	return TempDir;
}

FString& FOasisAssetData::GetImportSessionTempDir()
{
	const FString ProjectIntermediateDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectIntermediateDir());
	static FString SessionTempDir = FPaths::Combine(ProjectIntermediateDir, TEXT("SpyEngine"), TEXT("Import"), TEXT("Sessions"));
	return SessionTempDir;
}

FString FOasisAssetData::GetZipExportSessionTempDir()
{
	const FString ProjectIntermediateDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectIntermediateDir());
	const FString SessionId = FGuid::NewGuid().ToString(EGuidFormats::Digits);
	FString SessionTempDir = FPaths::Combine(ProjectIntermediateDir, TEXT("SpyEngine"), TEXT("Export"), TEXT("Sessions"), SessionId);
	return SessionTempDir;
}

void FOasisAssetData::CheckIfCompatibleWithCurrentEngineVersion()
{
...
bool FOasisAssetData::IsCompatibleWithCurrentEngineVersion() const
{
	return bCompatible;
}

FString FOasisAssetData::GetInvalidReason() const
{
...
void FOasisAssetData::PrintAssetData() const
{
	ECB_LOG(Display, TEXT("    FOasisAssetData for %s"), *PackageFilePath.ToString());
...
bool FOasisAssetData::IsUAsset() const
{
...
bool FOasisAssetData::IsTopLevelAsset() const
{
...
bool FOasisAssetData::IsUMap() const
{
...
bool FOasisAssetData::GetTagValue(const FName InTagName, ValueType& OutTagValue) const
{
...
void FExtAssetRegistry::BroadcastAssetUpdatedEvent(const FOasisAssetData& UpdatedAsset)
{
...
bool FExtAssetRegistry::GetAndTrimAssetGatherResult()
{
	...
	if (BackgroundAssetGatherer.IsValid())
	{
		...
		if (bHasGatherResult)
		{
			const TMap<FName, FOasisAssetData>& ParsedAssets = GatherResult.GetParsedAssets();
			for (const auto& Pair : ParsedAssets)
			{
				const FName& FilePath = Pair.Key;
				const FOasisAssetData& AssetData = Pair.Value;

				ECB_LOG(Display, TEXT("BackgroundAssetGatherer for %s, bIsValide?: %d"), *FilePath.ToString(), AssetData.IsValid());

				FOasisAssetData** FoundAssetData = State.CachedAssetsByFilePath.Find(FilePath);
				...
				if (!bAlreadyCached)
				{
					FOasisAssetData* NewAssetData = new FOasisAssetData(AssetData);
...
// 						
// 						TArray<FOasisAssetData*>& CachedAssets = State.CachedAssetsByFolder.FindOrAdd(*Dir);
...
bool FExtAssetRegistry::GetAssets(const struct FARFilter& InFilter, TArray<FOasisAssetData>& OutAssetData)
{
	...
	TArray<TSet<FOasisAssetData*>> DiskFilterSets;
	...
	const bool bIgnoreInvalidAssets = !OasisContentBrowserSetting->DisplayInvalidAssets;

	// On disk package paths
	if (FilterPackagePaths.Num())
	{
		TSet<FOasisAssetData*>& PathFilter = DiskFilterSets[DiskFilterSets.AddDefaulted()];
		{
			TArray<FOasisAssetData*> FilteredAssets;
			for (FName PackagePath : FilterPackagePaths)
			{
				const TArray<FOasisAssetData*> PathAssets = GetCachedAssetsByFolder(PackagePath);
				if (PathAssets.Num() > 0)
				{
					for (FOasisAssetData* AssetData : PathAssets)
					{
	...
	{
		TSet<FOasisAssetData*>& ClassFilter = DiskFilterSets[DiskFilterSets.AddDefaulted()];
		...
		for (FTopLevelAssetPath ClassPath : FilterClassPaths)
		{
			...
			const TArray<FOasisAssetData*> ClassAssets = GetCachedAssetsByClass(ClassName);
			if (ClassAssets.Num() > 0)
			{
				for (FOasisAssetData* AssetData : ClassAssets)
				{
#if ECB_FEA_SHOW_INVALID
					const bool bShouldIgnore = bIgnoreInvalidAssets && !AssetData->IsValid();
#else
	...
	{
		TSet<FOasisAssetData*>& PackageNameFilter = DiskFilterSets[DiskFilterSets.AddDefaulted()];
		...
		for (FName PackageName : FilterPackageNames)
		{
			...
			if (FilePath != nullptr)
			{
				FOasisAssetData* AssetData = GetCachedAssetByFilePath(*FilePath);
	...
	if (DiskFilterSets.Num() > 0)
	{
		...
		TSet<FOasisAssetData*>* CombinedFilterSetPtr = &DiskFilterSets[0];
		TSet<FOasisAssetData*> IntersectedFilterSet;
		...
		if (DiskFilterSets.Num() > 1)
		{
			...
			for (int32 SetIdx = 1; SetIdx < DiskFilterSets.Num() && IntersectedFilterSet.Num() > 0; ++SetIdx)
			{
				...
				TSet<FOasisAssetData*> OtherFilterSet = DiskFilterSets[SetIdx];
		...
		for (FOasisAssetData* AssetData : *CombinedFilterSetPtr)
		{
...
void FExtAssetRegistry::RunAssetsThroughFilter(TArray<FOasisAssetData>& AssetDataList, const FARFilter& Filter) const
{
	if (!Filter.IsEmpty())
	{
		...
		{
			const FOasisAssetData& AssetData = AssetDataList[AssetDataIdx];
...
bool FExtAssetRegistry::GetDependencies(FName PackageName, TArray<FName>& OutDependencies, UE::AssetRegistry::EDependencyCategory InDependencyType /*= UE::AssetRegistry::EDependencyCategory::Package*/, const UE::AssetRegistry::FDependencyQuery& Flags)
{
	...
	{
		FOasisAssetData& AssetData = AssetDataList[0];
		if (AssetData.IsValid() && AssetData.HasContentRoot())
		{
			TSet<FName> Dependencies;
			bool bHard = !!(Flags.Required & UE::AssetRegistry::EDependencyProperty::Hard);
			if (bHard)
			{
			...
			if (AssetContentType == FOasisAssetData::EContentType::Plugin && !AssetContentDir.IsEmpty())
			{
				GameRoot = FOasisAssetDataUtil::GetPluginNameFromAssetContentRoot(AssetContentDir);
			}

			for (const FName Depend : Dependencies)
			{
				FString RemappedFilePath;
				if (FOasisAssetDataUtil::RemapGamePackageToFullFilePath(GameRoot, Depend.ToString(), AssetContentDir, RemappedFilePath))
				{
					FString RemappedPackageName = FOasisAssetData::ConvertFilePathToTempPackageName(RemappedFilePath);
...
FPrimaryAssetId FExtAssetRegistry::ExtractPrimaryAssetIdFromFakeAssetData(const FOasisAssetData& InAssetData)
{
...
void FExtAssetRegistry::ExtractAssetIdentifiersFromAssetDataList(const TArray<FOasisAssetData>& AssetDataList, TArray<FExtAssetIdentifier>& OutAssetIdentifiers)
{
...
{	
	...
	else
	{
		...
		GConfig->GetArray(*SOasisContentBrowser::SettingsIniSection, TEXT("RootContentPaths"), RootContentPaths, GEditorPerProjectIni);
	}
...
void FExtAssetRegistry::MergeRootContentPathsWith(const TArray<FString>& InPathsToMerge, bool bReplaceCurrent)
{
	...
	const UOasisContentBrowserSettings* OasisContentBrowserSetting = GetDefault<UOasisContentBrowserSettings>();
	const bool bCacheMode = OasisContentBrowserSetting->bCacheMode;
...
void FExtAssetRegistry::SaveRootContentPaths()
{
	...
	const UOasisContentBrowserSettings* OasisContentBrowserSetting = GetDefault<UOasisContentBrowserSettings>();
	GConfig->SetArray(*SOasisContentBrowser::SettingsIniSection, TEXT("RootContentPaths"), RootContentPaths, GEditorPerProjectIni);
...
bool FExtAssetRegistry::QueryRootContentPathInfo(const FString& InRootContentPath, FText* OutDisplayName, FOasisAssetData::EContentType* OutContentType, FString* OutAssetContentRoot) const
{
...
bool FExtAssetRegistry::ParseAssetContentRoot(const FString& InFilePath, FString& OutRelativePath, FString& OutAssetContentRoot, FOasisAssetData::EContentType& OutAssetContentType, TSet<FName>* InAllDependencies)
{
	...
	else
	{
		...
		if (!bFoundRootPath)
		{
			...
			if (bFoundRootPath)
			{
				...
				OutAssetContentType = FOasisAssetData::EContentType::Plugin;
			}
		}
#endif
		...
		if (!bFoundRootPath)
		{
			...
			{
				AssetContentRootHost = AssetContentRoot / TEXT("..");
				OutAssetContentType = FOasisAssetData::EContentType::Project;
			}
		}
		...
		if (!bFoundRootPath)
		{
			...
			{
				AssetContentRootHost = AssetContentRoot / TEXT("../..");
				OutAssetContentType = FOasisAssetData::EContentType::VaultCache;
			}
		}
		...
		if (!bFoundRootPath)
		{
			...
			{
				AssetContentRootHost = AssetContentRoot / TEXT("..");
				OutAssetContentType = FOasisAssetData::EContentType::PartialProject;
			}
		}
		...
		if (!bFoundRootPath)
		{
			...
			if (QueryRootContentPathFromFilePath(InFilePath, RootContentPath) && InAllDependencies)
			{
				...
				FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedSubPaths(*RootContentPath, SubPaths, /*bRecursively = */ true);
				...
				for (const FName& Dir : SubPaths)
				{
					...
					if (InAllDependencies->Num() > 0)
					{
						for (const FName& DependPackageName : *InAllDependencies)
						{
							...
							FString PackageRoot = FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(DependPackageString);
							FString DependentFilePath;
							if (FOasisAssetDataUtil::RemapGamePackageToFullFilePath(PackageRoot, DependPackageString, AssetContentRoot, DependentFilePath))
							{
				...
				if (bFoundRootPath)
				{
					...
					OutAssetContentType = FOasisAssetData::EContentType::Orphan;
				}
			}
		}
#endif
...
void FExtAssetRegistry::AddAssetContentRoot(const FString& InAssetContentRoot, const FString& InAssetContentRootHost, const FString& InConfigDir, FOasisAssetData::EContentType InContentType)
{
...
FOasisAssetData::EContentType FExtAssetRegistry::GetAssetContentRootContentType(const FString& InAssetContentRoot) const
{
	TMap<FName, FOasisAssetData::EContentType> CachedAssetContentType;
	FName AssetContentRootName = *InAssetContentRoot;
	if (const FOasisAssetData::EContentType* ContentType = State.CachedAssetContentType.Find(AssetContentRootName))
	{
		return *ContentType;
	}
	return FOasisAssetData::EContentType::Unknown;
}
...
void FExtAssetRegistry::CacheAssets(const struct FARFilter& InFilter)
{
	...
	TSet<FOasisAssetData*> FilteredAssets;

	{
		...
		if (!bPrepareTaskCanceled && FilterPackagePaths.Num())
		{
			...
			for (FName PackagePath : FilterPackagePaths)
			{
				...
				if (const TArray<FOasisAssetData*>* PathAssets = GetOrCacheAssetsByFolder(PackagePath, bDealyParse).Find(PackagePath))
				{
		...
		if (!bPrepareTaskCanceled && FilterClassNames.Num())
		{
			...
			for (const FName& PackagePath : FilterPackagePaths)
			{
				...
				for (const FName& SubPath : SubPaths)
				{
					...
					if (const TArray<FOasisAssetData*>* SubPathAssets = GetOrCacheAssetsByFolder(SubPath, bDealyParse).Find(SubPath))
					{
		...
		if (!bPrepareTaskCanceled && FilterPackageNames.Num())
		{
			...
			for (FName PackageName : FilterPackageNames)
			{
				...
				if (FilePath != nullptr)
				{
					if (FOasisAssetData* AssetData = GetOrCacheAssetByFilePath(*FilePath, bDealyParse))
					{
						FilteredAssets.Add(AssetData);
					}
				}
	...
	// Parsing
	TSet<FOasisAssetData*> ToParseAssets;
	for (FOasisAssetData* AssetData : FilteredAssets)
	{
	...
	if (NumToParseAssets > 0)
	{
		...
		TSharedPtr<SWidget> PreviousFocusedWidget = FSlateApplication::Get().GetKeyboardFocusedWidget();
		{
			...
			for (FOasisAssetData* AssetData : ToParseAssets)
			{
				AssetData->Parse();
...
void FExtAssetRegistry::CacheAssetsAsync(const struct FARFilter& InFilter)
{
	...
	if (FilteredAssetPaths.Num() > 0)
	{
		...
		{
			if (FOasisAssetData* AssetData = GetCachedAssetByFilePath(AssetPathName))
			{
				if (AssetData->IsParsed())
				{
					continue;
...
void FExtAssetRegistry::GetOrCacheSubPaths(const FName& InBasePath, TSet<FName>& OutSubPaths, bool bInRecurse)
{
	...
	if (bInRecurse)
	{
		...
		//FOasisPackageUtils::GetDirectoriesRecursively(InBasePath.ToString(), SubPaths);
	}
	else
	{
		TArray<FString> SubPaths;
		FOasisPackageUtils::GetDirectories(InBasePath.ToString(), SubPaths);
...
FExtAssetDependencyInfo FExtAssetRegistry::GetOrCacheAssetDependencyInfo(const FOasisAssetData& InAssetData, bool bShowProgess)
{
	...
	if (!InAssetData.IsUAsset())
	{
		...
		{
			DependencyInfo.AssetStatus = FOasisPackageUtils::DoesPackageExist(InAssetData.PackageName.ToString()) ? EDependencyNodeStatus::Valid : EDependencyNodeStatus::Missing;
		}
		else
		{
...
const FExtAssetDependencyInfo* FExtAssetRegistry::GetCachedAssetDependencyInfo(const FOasisAssetData& InAssetData) const
{
	const FName PackageFilePath = InAssetData.PackageFilePath;
...
bool FExtAssetRegistry::RemoveCachedAssetDependencyInfo(const FOasisAssetData& InAssetData)
{
	auto& CachedDependencyInfoByFilePath = State.CachedDependencyInfoByFilePath;
	const FName PackageFilePath = InAssetData.PackageFilePath;
...
bool FExtAssetRegistry::IsCachedDependencyInfoInValid(const FOasisAssetData& InAssetData, bool bTreatSoftErrorAsInvalid /*= false*/) const
{
...
FOasisAssetData* FExtAssetRegistry::GetOrCacheAssetByFilePath(const FName& InFilePath, bool bDelayParse/* = false*/)
{
	...
	if (!CachedAssetsByFilePath.Contains(InFilePath))
	{
		...
		// New Asset Data
		FOasisAssetData* NewAssetData = new FOasisAssetData(InFilePath.ToString(), bDelayParse);
	...
	FOasisAssetData** FoundAssetData = CachedAssetsByFilePath.Find(InFilePath);
	if (FoundAssetData)
	{
...
TMap<FName, TArray<FOasisAssetData*>>& FExtAssetRegistry::GetOrCacheAssetsByFolder(const FName& InFolder, bool bDelayParse)
{
	auto& CachedAssetsByFolder = State.CachedAssetsByFolder;

	if (!CachedAssetsByFolder.Contains(InFolder))
	{
		TArray<FOasisAssetData*> Assets;
		{
			...
			if (!CachedFilePathsByFolder.Contains(InFolder))
			{
				...
				FOasisPackageUtils::GetAllPackages(InFolder, PathList);
			...
			{
				FOasisAssetData* CachedAsset = GetOrCacheAssetByFilePath(FilePath, bDelayParse);
...
TMap<FName, TArray<FOasisAssetData*>>& FExtAssetRegistry::GetOrCacheAssetsByClass(const TSet<FName>& InPaths, bool bRecursively)
{
...
FOasisAssetData* FExtAssetRegistry::GetCachedAssetByFilePath(const FName& InFilePath)
{
	auto& CachedAssetsByFilePath = State.CachedAssetsByFilePath;
	FOasisAssetData** FoundAssetData = CachedAssetsByFilePath.Find(InFilePath);
	if (FoundAssetData)
...
FOasisAssetData* FExtAssetRegistry::GetCachedAssetByPackageName(const FName& InPackageName)
{
...
TArray<FOasisAssetData*> FExtAssetRegistry::GetCachedAssetsByFolder(const FName& InDirectory)
{
	if (State.CachedAssetsByFolder.Find(InDirectory))
	{
		return State.CachedAssetsByFolder[InDirectory];
	}
	return TArray<FOasisAssetData*>();
}

TArray<FOasisAssetData*> FExtAssetRegistry::GetCachedAssetsByClass(const FName& InClass)
{
...
bool FExtAssetRegistry::IsEmptyFolder(const FString& FolderPath)
{
	...
	bool bEmptyFolder = !FOasisPackageUtils::HasPackages(FolderPath, /*bRecursively*/ true);
...
void FExtAssetRegistry::CacheRootContentPathInfo()
{
	...
	{
		FOasisAssetData::EContentType ContentType = FOasisAssetData::EContentType::Orphan;
		...
		if (DisplayFolderPath.EndsWith(VaultCacheContentFolderName))
		{
			...
			if (FExtContentDirFinder::FindWithFolder(VaultCacheFolder, TEXT("manifest"), /*bExtension*/ false))
			{
				...
				ContentType = FOasisAssetData::EContentType::VaultCache;
			}
		}
		...
		else if (DisplayFolderPath.EndsWith(ProjectOrPluginContentFolderName))
		{
			...
			if (FExtContentDirFinder::FindWithFolder(ProjectOrPluginFolder, TEXT(".uplugin"), /*bExtension*/ true))
			{
				...
				ContentType = FOasisAssetData::EContentType::Plugin;
			}
			else if (FExtContentDirFinder::FindWithFolder(ProjectOrPluginFolder, TEXT(".uproject"), /*bExtension*/ true))
			{
				DisplayFolderPath = ProjectOrPluginFolder;
				ContentType = FOasisAssetData::EContentType::Project;
			}
		}
		////////////////////////////////////////////
		// 
		...
		// Partial Project
		if (ContentType == FOasisAssetData::EContentType::Orphan && FExtContentDirFinder::FindFolder(DisplayFolderPath, TEXT("Content"), TmpAssetContentRoot, TmpRelativePath))
		{
			...
			ContentType = FOasisAssetData::EContentType::PartialProject;
		}
...
TOptional<FAssetImportInfo> FExtAssetRegistry::ExtractAssetImportInfo(const FOasisAssetData& AssetData) const
{
...
void FExtAssetRegistry::PrintCacheStatus()
{
	...
	ECB_LOG(Display, TEXT("  ================================="));
	ECB_LOG(Display, TEXT("  MaxNumThumbnailsInPool:      %d"), FOasisContentBrowserSingleton::GetThumbnailPool().NumInPool);
	ECB_LOG(Display, TEXT("  Num Used in Pool:            %d"), FOasisContentBrowserSingleton::GetThumbnailPool().Pool.Num());
	ECB_LOG(Display, TEXT("  Pool Memory Ussage:         %dk"), FOasisContentBrowserSingleton::GetThumbnailPool().GetAllocatedSize() / 1024);
	ECB_LOG(Display, TEXT("  ---------------------------------"));
...
void FExtAssetRegistry::StartFolderGathering(const FString& InGatheringFolder)
{
	...
	const UOasisContentBrowserSettings* OasisContentBrowserSetting = GetDefault<UOasisContentBrowserSettings>();
	TArray<FString> IgnoreFolders = OasisContentBrowserSetting->IgnoreFolders;
	if (OasisContentBrowserSetting->bIgnoreCommonNonContentFolders)
	{
		IgnoreFolders.Append(OasisContentBrowserSetting->CommonNonContentFolders);
	}
	if (OasisContentBrowserSetting->bIgnoreExternalContentFolders)
	{
		IgnoreFolders.Append(OasisContentBrowserSetting->ExternalContentFolders);
	}
	FExtFolderGatherPolicy GatherPolicy(OasisContentBrowserSetting->bIgnoreFoldersStartWithDot, IgnoreFolders);
#endif
...
//
#if 0
bool FOasisAssetDataUtil::RemapPackagesToFullFilePath(const FString& InPackageName, const TArray<FString>& InRootPackageNamesToReplace, const FString& InAssetContentDir, FString& OutFullFilePath)
{
...
FString FOasisAssetDataUtil::GetPluginNameFromAssetContentRoot(const FString& InAssetContentRoot)
{
...
FString FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(const FString& InFullPackagePath)
{
	auto IsEndOfSearchChar = [](TCHAR C) { return (C == TEXT('/')) || (C == TEXT('\\')); };
...
bool FOasisAssetDataUtil::RemapGamePackageToFullFilePath(const FString& GameRoot, const FString& InPackageName, const FString& InAssetContentDir, FString& OutFullFilePath, bool bPackageIsFolder)
{
...
bool FOasisAssetDataUtil::SyncToContentBrowser(const FOasisAssetData& InAssetData)
{
...
EDependencyNodeStatus FExtAssetDependencyWalker::GatherDependencies(const FOasisAssetData& InAssetData, const FString& InAssetContentDir, const FOasisAssetData::EContentType& InAssetContentType, FExtAssetDependencyInfo& InOuDependencyInfo, int32 Level, FScopedSlowTask* SlowTaskPtr)
{
	FString Padding; 
#if ECB_DEBUG
...
EDependencyNodeStatus FExtAssetDependencyWalker::GatherDependencies(const FOasisAssetData& InAssetData, const FString& InAssetContentDir, const FOasisAssetData::EContentType& InAssetContentType, FExtAssetDependencyInfo& InOuDependencyInfo, bool bShowProgess)
{
	if (InAssetData.IsValid())
	{
...
EDependencyNodeStatus FExtAssetDependencyWalker::GatherPackageDependencies(const FName& DependPackageName, FExtAssetDependencyInfo& InOuDependencyInfo, const FString& InAssetContentDir, const FOasisAssetData::EContentType& InAssetContentType, int32& Level, bool bSoftReference, FScopedSlowTask* SlowTaskPtr)
{
	...
	FString GameRoot(TEXT("/Game/"));
#if ECB_FEA_IMPORT_PLUGIN
	if (InAssetContentType == FOasisAssetData::EContentType::Plugin && !InAssetContentDir.IsEmpty())
	{
		GameRoot = FOasisAssetDataUtil::GetPluginNameFromAssetContentRoot(InAssetContentDir);
	}
#endif
	...
	if (DependPackageString.StartsWith(GameRoot))
	{
		...
		FString DependentFilePath;
		if (!FOasisAssetDataUtil::RemapGamePackageToFullFilePath(GameRoot, DependPackageString, InAssetContentDir, DependentFilePath))
		{
		...
		{
			if (FOasisAssetData* DependentAssetPtr = FOasisContentBrowserSingleton::GetAssetRegistry().GetOrCacheAssetByFilePath(FName(*DependentFilePath)))
			{
				FOasisAssetData& DependentAsset = *DependentAssetPtr;
	...
	{
		const bool bPackageExist = FOasisPackageUtils::DoesPackageExist(DependPackageString);
		if (!bPackageExist)
		{
...
//
bool FExtAssetValidator::ValidateDependency(const TArray<FOasisAssetData>& InAssetDatas, FString* OutValidateResult, bool bShowProgess, EDependencyNodeStatus* OutAssetStatus)
{
	int32 NumValid = 0;
	int32 NumInValid = 0;
	...
	for (const FOasisAssetData& InAssetData : InAssetDatas)
	{
		bool bValid = false;
		...
		{
			FExtAssetDependencyInfo DependencyInfo = FOasisContentBrowserSingleton::GetAssetRegistry().GetOrCacheAssetDependencyInfo(InAssetData, bShowProgess);
...
bool FExtAssetValidator::ValidateDependency(const TArray<FOasisAssetData*>& InAssetDatas, FString* OutValidateResultPtr /*= nullptr*/, bool bShowProgess /*= false*/, EDependencyNodeStatus* OutAssetStatus /*= nullptr*/)
{
	TArray<FOasisAssetData> AssetDatas;
	for (const FOasisAssetData* AssetDataPtr : InAssetDatas)
	{
...
void FExtAssetValidator::InValidateDependency(const TArray<FOasisAssetData>& InAssetDatas)
{
	for (const FOasisAssetData& InAssetData : InAssetDatas)
	{
		FOasisContentBrowserSingleton::GetAssetRegistry().RemoveCachedAssetDependencyInfo(InAssetData);
	}
}
...
struct FExtAssetImportTargetFolderInfo
{
	...
	{
		TSharedPtr<FLinearColor> ExistingColor = OasisContentBrowserUtils::LoadColor(TargetFolderPacakgeName, /*bNoCache*/ true);
...
struct FExtAssetImportTargetFileInfo
{
	...
	{
		if (FOasisAssetData* SourceAssetData = FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetByFilePath(*InSourceFile))
		{
...
struct FExtAssetImportInfo
{
	 ...
	 */
	void GatherTargetFileInfos(const FOasisAssetData& AssetData, const FString& AssetContentDir, const FString& TargetContentRoot, const FUAssetImportSetting& InImportSetting)
	{
		...
		const bool bImportFolderColor = !bFlatMode && !bSandboxMode && !bExportMode && InImportSetting.bImportFolderColor && FOasisContentBrowserSingleton::GetAssetRegistry().IsAsetContentRootHasFolderColor(AssetContentDir);
		...
		{
			FOasisContentBrowserSingleton::GetAssetRegistry().GetAssetContentRootColoredFolders(AssetContentDir, ColoredFolders);
		}

		DependencyInfo = FOasisContentBrowserSingleton::GetAssetRegistry().GetOrCacheAssetDependencyInfo(AssetData);

		const FString GameRootWithSlash(TEXT("/Game/"));
		const FString SandboxRootWithSlash = FOasisAssetData::GetSandboxPackagePathWithSlash();
		...
		{
			if (FOasisAssetData* DependentAssetPtr = FOasisContentBrowserSingleton::GetAssetRegistry().GetOrCacheAssetByFilePath(FName(*ValidDependentFile)))
			{
				FOasisAssetData& DependentAsset = *DependentAssetPtr;
...
#if ECB_WIP_IMPORT_ORPHAN
				if (!DependentAsset.HasContentRoot() && DependentAsset.AssetContentType == FOasisAssetData::EContentType::Orphan)
				{
					const bool bHasCachedCotentRoot = FOasisContentBrowserSingleton::GetAssetRegistry().GetAssetContentRoot(DependentAsset.PackageFilePath.ToString(), OrphanAssetContentRoot);
					bHasContentRootForDirectCopyOrphanAsset = InImportSetting.bDirectCopyMode && DependentAsset.AssetContentType == FOasisAssetData::EContentType::Orphan;
				...
				if (bHasCotentRoot)
				{
					...
					const bool bPluginAsset = AssetContentType == FOasisAssetData::EContentType::Plugin && !AssetContentDir.IsEmpty();
					FString PluginRootWithSlash = bPluginAsset 
						? FOasisAssetDataUtil::GetPluginNameFromAssetContentRoot(AssetContentRoot) // /PluginName/
						: TEXT("");
					...
					// if (bPluginAsset || bFlatMode || bSandboxMode)
					{
						...
						if (!bRedirectMe)
						{
							FString SourceRootPackage = FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(SourcePackgeName);
							FString TargetRootPackage = FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(TargetPackageName);
							...
							for (int32 Index = ColoredFolders.Num() - 1; Index >= 0;  --Index)
							{
								...
								if (SourceFilePath.StartsWith(ColoredFolderStr))
								{
									...
									FString TargetPackageRoot = bRedirectMe
										? FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(RedirectToPackageName)
										: FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(TargetPackageName);
									FString TargetFolderPackagePath = TargetPackageRoot + FolderRelativePath;
									
									FLinearColor FolderColor;
									if (!FOasisContentBrowserSingleton::GetAssetRegistry().GetFolderColor(ColoredFolderStr, FolderColor))
									{
										continue;
									}
...
struct FExtAssetRollbackInfo
{
	...
	void RollbackAll()
	{
		...
		for (auto It = TargetToBackupMap.CreateIterator(); It; ++It)
		{
			...
			{	
				FOasisPackageUtils::UnloadPackage({ Target.ToString() }, &ReplacedPackages);
...
public:
	...
	// Source
	FOasisAssetData SourceAssetData;
...
struct FExtAssetImportSession
{
	FExtAssetImportSession(const FUAssetImportSetting& InImportSetting)
	{
		...
		const FString SessionId = FGuid::NewGuid().ToString(EGuidFormats::Digits);
		SesstionTempDir = FPaths::Combine(FOasisAssetData::GetImportSessionTempDir(), SessionId);
	...
	void ImportAssets(const TArray<FOasisAssetData>& InAssetDatas)
	{
		...
		ECB_LOG(Display, TEXT("------------ Phase 1: Gathering dependencies -----------------"));
		{
			...
			if (IsSessionRequestToCancel())
			{
				...
				OasisContentBrowserUtils::NotifyMessage(FText::FromString(AbortMessage));
				return;
			}
	...
	void PrepareImportTasksWithDependencies(const TArray<FOasisAssetData>& InAssetDatas)
	{
		...
		{
			const FOasisAssetData& AssetData = InAssetDatas[Index];
			...
			{
				FExtAssetDependencyInfo DependencyInfo = FOasisContentBrowserSingleton::GetAssetRegistry().GetOrCacheAssetDependencyInfo(AssetData, /*bShowProgess*/ true);
				if (DependencyInfo.AssetStatus == EDependencyNodeStatus::AbortGathering)
				{
			...
			else
			{
				...
				FString RootContentPath;
				if (/*ImportSetting.bDirectCopyMode && */AssetData.AssetContentType == FOasisAssetData::EContentType::Orphan
					&& FOasisContentBrowserSingleton::GetAssetRegistry().QueryRootContentPathFromFilePath(AssetData.PackageFilePath.ToString(), RootContentPath)
					)
				{
					FExtAssetImportInfo& ImportInfo = NewTask.ImportInfo;
...
#else
						OasisContentBrowserUtils::SaveColor(TargetFolderInfo.TargetFolderPacakgeName, nullptr); // clear PatchColors cache
						OasisContentBrowserUtils::SaveColor(TargetFolderInfo.TargetFolderPacakgeName, MakeShareable(new FLinearColor(TargetFolderInfo.TargetFolderColor)), true);
#endif
	...
	void NotifyResult()
	{
		...
		OasisContentBrowserUtils::NotifyMessage(FText::FromString(ResultMessage), /*bPrintToConsole*/true);
	}
...
// 		bool bResult = IFileManager::Get().Move(/*Dest=*/*InBackup.ToString(), /*Src=*/ *InTarget.ToString(), bReplace, bEvenIfReadOnly, bAttributes, bDoNotRetryOrError);
	...
	bool AddRollbackNewFile(FExtAssetImportTask& InTask, const FName& InSource, const FName& InTarget, bool bReplaceExisting, FString* InPluginRootForReloadPackage /*todo: include all invalid roots? */)
	{
		...
		if (bReplaceExisting)
		{
			FOasisPackageUtils::UnloadPackage({ InTarget.ToString() }, &ReplacedPackages);
		}
			...
			struct Local
			{
				static void GetInvalidRoots(const TArray<UPackage*>& InReplacedPackages, TArray<FString>& OutInValidRootsWithSlash)
				{
					for (const UPackage* Package : InReplacedPackages)
					{
						...
						FString InvalidRoot = FOasisAssetDataUtil::GetPackageRootFromFullPackagePath(PackagePath);
				...
				static void MountInvalidRoots(const TArray<FString>& InValidRootsWithSlash)
				{
					...
					{
						FPackageName::RegisterMountPoint(MountPoint /*TEXT("/SomeRoot/")*/, FOasisAssetData::GetSpyEngineTempDir());
					}
				}
				...
				static void UnMountInvalidRoots(const TArray<FString>& InValidRootsWithSlash)
				{
					for (const FString& MountPoint : InValidRootsWithSlash)
					{
						FPackageName::UnRegisterMountPoint(MountPoint /*TEXT("/SomeRoot/")*/, FOasisAssetData::GetSpyEngineTempDir());
					}
				}
...
private:
	...
	struct FRedirectHelper
	{
		...
		TMap<FSoftObjectPath, FSoftObjectPath> RedirectMap;// /Game/SomePackage.SomeAssetName => /SpyEngine/Sandbox/SomePackage.SomeAssetName
...
void FExtAssetImporter::ImportAssets(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& ImportSetting)
{
...
void FExtAssetImporter::DoImportAssets(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& ImportSetting)
{
	int32 NumMainAssetsToImport = InAssetDatas.Num();
	ECB_LOG(Display, TEXT("========== Importing %d Assets ============"), NumMainAssetsToImport);
	if (NumMainAssetsToImport <= 0)
...
void FExtAssetImporter::ImportAssetsByFilePaths(const TArray<FString>& InAssetFilePaths, const FUAssetImportSetting& ImportSetting)
{
	...
	{
		FOasisAssetData* AssetData = FOasisContentBrowserSingleton::GetAssetRegistry().GetOrCacheAssetByFilePath(*AssetFilePath, /*bDelayParse */false);
		if (AssetData && AssetData->IsValid())
		{
...
void FExtAssetImporter::ImportAssetsToFolderPackagePath(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& InImportSetting, const FString& InDestPackagePath)
{
...
void FExtAssetImporter::ImportAssetsWithPathPicker(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& InImportSetting)
{
...
void FExtAssetImporter::ImportProjectFolderColors(const FString& InRootOrHost)
{
	...
	if (FOasisContentBrowserSingleton::GetAssetRegistry().IsRootFolder(InRootOrHost))
	{
		FOasisContentBrowserSingleton::GetAssetRegistry().QueryRootContentPathInfo(InRootOrHost, nullptr, nullptr, &AssetContentDir);
	}
	else if (const FName* AssetContentDirPtr = FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetContentRootHostDirs().Find(*InRootOrHost))
	{
	...
	{
		if (const FName* ConfigDirName = FOasisContentBrowserSingleton::GetAssetRegistry().GetCachedAssetContentRootConfigDirs().Find(*AssetContentDir))
		{
...
#else
						OasisContentBrowserUtils::SetPathColor(PackagePath, nullptr); // clear PatchColors cache
						OasisContentBrowserUtils::SetPathColor(PackagePath, MakeShareable(new FLinearColor(FolderColor)), true);
#endif
					}
					else
					{
...
void FExtAssetImporter::ExportAssets(const TArray<FOasisAssetData>& InAssetDatas)
{
...
// 				Session.PostImport();
				...
				const UOasisContentBrowserSettings* OasisContentBrowserSetting = GetDefault<UOasisContentBrowserSettings>();
				if (/*!Session.IfAllTasksFailed() && */ OasisContentBrowserSetting->bOpenFolderAfterExport)
				{
...
void FExtAssetImporter::ZipUpSourceAssets(const TArray<FOasisAssetData>& InAssetDatas)
{
	int32 NumAssetsToExport = InAssetDatas.Num();
	ECB_LOG(Display, TEXT("========== Exporting %d Assets ============"), NumAssetsToExport);
	if (NumAssetsToExport <= 0)
	...
	if (bOpened && SaveFilenames.Num() > 0)
	{
...

		{
			...
			ExportSetting.TargetContentDir = FOasisAssetData::GetZipExportSessionTempDir();
...
bool FExtAssetImporter::ValidateImportSetting(const FUAssetImportSetting& ImportSetting)
{
	...
	{
		OasisContentBrowserUtils::NotifyMessage(ImportSetting.GetInvalidReason());
		return false;
	}

	if (ImportSetting.bImportToPluginFolder && ImportSetting.bFlatMode)
	{
		OasisContentBrowserUtils::NotifyMessage(LOCTEXT("", "Flat Import to Plugin is not supported!"));
		return false;
	}
...
FUAssetImportSetting FUAssetImportSetting::GetDefaultExportSetting(const FString& InTargetDir)
{
	...
	ImportSetting.bSkipImportIfAnyDependencyMissing = OasisContentBrowserSetting->bSkipExportIfAnyDependencyMissing;
#if ECB_FEA_IMPORT_IGNORE_SOFT_ERROR
	ImportSetting.bIgnoreSoftReferencesError = OasisContentBrowserSetting->bExportIgnoreSoftReferencesError;
#endif
	ImportSetting.bOverwriteExistingFiles = OasisContentBrowserSetting->bExportOverwriteExistingFiles;
	ImportSetting.bRollbackIfFailed = OasisContentBrowserSetting->bRollbackExportIfFailed;
...
FUAssetImportSetting FUAssetImportSetting::GetSavedImportSetting()
{
	...
	FUAssetImportSetting ImportSetting;
	ImportSetting.bSkipImportIfAnyDependencyMissing = OasisContentBrowserSetting->bSkipImportIfAnyDependencyMissing;
	ImportSetting.bOverwriteExistingFiles = OasisContentBrowserSetting->bImportOverwriteExistingFiles;
	ImportSetting.bRollbackIfFailed = OasisContentBrowserSetting->bRollbackImportIfFailed;
	ImportSetting.bSyncAssetsInContentBrowser = OasisContentBrowserSetting->bImportSyncAssetsInContentBrowser;
	ImportSetting.bSyncExistingAssets = OasisContentBrowserSetting->bImportSyncExistingAssets;
	ImportSetting.bLoadAssetAfterImport = OasisContentBrowserSetting->bLoadAssetAfterImport;
#if ECB_WIP_IMPORT_ADD_TO_COLLECTION
	ImportSetting.bAddImportedAssetsToCollection = OasisContentBrowserSetting->bAddImportedAssetsToCollection;
	ImportSetting.bUniqueCollectionNameForEachImportSession = OasisContentBrowserSetting->bUniqueCollectionNameForEachImportSession;
	ImportSetting.ImportedUAssetCollectionName = OasisContentBrowserSetting->DefaultImportedUAssetCollectionName;
#endif

#if ECB_WIP_IMPORT_TO_PLUGIN_FOLDER
	ImportSetting.bImportToPluginFolder = OasisContentBrowserSetting->bImportToPluginFolder;
	ImportSetting.bWarnBeforeImportToPluginFolder = OasisContentBrowserSetting->bWarnBeforeImportToPluginFolder;
	ImportSetting.ImportToPluginName = OasisContentBrowserSetting->ImportToPluginName;
#endif

#if ECB_WIP_IMPORT_FOLDER_COLOR
	ImportSetting.bImportFolderColor = OasisContentBrowserSetting->bImportFolderColor;
#if ECB_WIP_IMPORT_FOLDER_COLOR_OVERRIDE
	ImportSetting.bOverrideExistingFolderColor = OasisContentBrowserSetting->bOverrideExistingFolderColor;
#else
...
#if ECB_FEA_IMPORT_IGNORE_SOFT_ERROR
	ImportSetting.bIgnoreSoftReferencesError = OasisContentBrowserSetting->bImportIgnoreSoftReferencesError;
#endif
...
FUAssetImportSetting FUAssetImportSetting::GetSandboxImportSetting()
{
	FUAssetImportSetting ImportSetting;
	{
		ImportSetting.TargetContentDir = FOasisAssetData::GetSandboxDir();
...
FUAssetImportSetting FUAssetImportSetting::GetSandboxImportForDumpSetting()
{
	FUAssetImportSetting ImportSetting = GetSandboxImportSetting();
	{
		...
		{
			const FString SessionId = FGuid::NewGuid().ToString(EGuidFormats::Digits);
			ImportSetting.TargetContentDir = FPaths::Combine(FOasisAssetData::GetSandboxDir(), SessionId);
		}
	}
	return ImportSetting;
}
...
void FExtAssetRegistryStateCache::CacheNewAsset(FOasisAssetData* InAssetToCache)
{
	// Add to CachedAssetsByFolder
	{
		const FString Dir = InAssetToCache->GetFolderPath();
		TArray<FOasisAssetData*>& CachedAssets = CachedAssetsByFolder.FindOrAdd(*Dir);
...
void FExtAssetRegistryStateCache::ReCacheParsedAsset(FOasisAssetData* InAssetToCache)
{
	if (!InAssetToCache->IsParsed())
	{
		return;
	...
	for (const FString& InRemovedFolder : InRemovedFolders)
	{
		...
		// exact match
		{
			for (const FName& FolderPath : AllFoldersToRemove)
			{
				...
				if (!OasisContentBrowserSetting->bKeepCachedAssetsWhenRootRemoved)
		...
		{
			TArray<FOasisAssetData*>& Assets = It->Value;
...
void FExtAssetRegistryStateCache::CacheValidAssetInfo(FOasisAssetData* InAssetToCache)
{
	...
	TArray<FOasisAssetData*>& ClassAssets = CachedAssetsByClass.FindOrAdd(InAssetToCache->AssetClass);
	...
	for (auto TagIt = InAssetToCache->TagsAndValues.CreateConstIterator(); TagIt; ++TagIt)
	{
		...
		TArray<FOasisAssetData*>& TagAssets = CachedAssetsByTag.FindOrAdd(Key);
...
void FExtFolderGatherer::GetChildDirectoriesAndPackages(const FName& InRoot, const FName& InParent, const TArray<FName>& InBaseDirs, TArray<FName>& OutSubPaths, TMap<FName, TArray<FName>>& OutRecusriveSubPaths, TMap<FName, TArray<FName>>& OutputFilePaths, TMap<FName, FPathTree>& OutputPathTrees, bool bFoundAssetContentRoot)
{
	...
	for (const auto& BaseDir : InBaseDirs)
	{
		...
		if (!bFoundAssetContentRoot)
		{
			...
			{
				FOasisAssetData::EContentType ContentType = FOasisAssetData::EContentType::Unknown;
...
void FExtAssetGatherResult::AddReuslt(const FName& InFilePath, const FOasisAssetData& InParsedAsset)
{
...
uint32 FExtAssetGatherer::Run()
{
	while (!bRequestToStop)
	{
		...
		if (NumLeftToGather > 0)
		{
			FOasisAssetData LocalAssetDataToParse;
...
bool FExtAssetGatherer::DoGatherAssetByFilePath(const FName& InFilePath, FOasisAssetData& OutOasisAssetData) const
{
	OutOasisAssetData = FOasisAssetData(InFilePath.ToString(), /*bDelayParse*/false);
	return OutOasisAssetData.IsParsed();
}
...
{
	FExtAssetRegistry& ExtAssetRegistry = FOasisContentBrowserSingleton::GetAssetRegistry();
	FName FolderName(*InFolder);
	if (const TArray<FOasisAssetData*>* AssetsByFolder = ExtAssetRegistry.GetOrCacheAssetsByFolder(FolderName, /*bDealyParse*/true).Find(FolderName))
	{
		for (const FOasisAssetData* AssetData : *AssetsByFolder)
		{
...
public:
	...
	void SerializeTagsAndBundles(FOasisAssetData& Out)
	{
...
public:
	...
	void SerializeTagsAndBundles(const FOasisAssetData& Out)
	{
...
void FOasisAssetData::SerializeForCache(Archive&& Ar)
{
	Ar << PackageFilePath;
	Ar << PackageName;
	Ar << PackagePath;
...
bool FExtAssetRegistryStateCache::Save(FArchive& OriginalAr)
{
	...
	for (TPair<FName, FOasisAssetData*>& Pair : CachedAssetsByFilePath)
	{
		FOasisAssetData& AssetData(*Pair.Value);
...
void FExtAssetRegistryStateCache::Load(Archive&& Ar, FExtAssetRegistryVersion::Type Version)
{
	...
	TArrayView<FOasisAssetData> PreallocatedAssetDataBuffer(new FOasisAssetData[LocalNumAssets], LocalNumAssets);
	PreallocatedAssetDataBuffers.Add(PreallocatedAssetDataBuffer.GetData());
	for (FOasisAssetData& NewAssetData : PreallocatedAssetDataBuffer)
	{
...
void FExtAssetRegistryStateCache::SetAssetDatas(TArrayView<FOasisAssetData> AssetDatas)
{
	...
	auto SetPathCache = [&]()
	{
		...
		for (FOasisAssetData& AssetData : AssetDatas)
		{
...
{
	const UOasisContentBrowserSettings* OasisContentBrowserSetting = GetDefault<UOasisContentBrowserSettings>();
	FString FullFilePath = OasisContentBrowserSetting->CacheFilePath.FilePath;
