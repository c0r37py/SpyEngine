...
#include "IOasisContentBrowserSingleton.h"
 ...
 */
class SOasisAssetView : public SCompoundWidget
{
public:
	...
	DECLARE_DELEGATE_OneParam(FOnAssetSelected, const FOasisAssetData& /*AssetData*/);

	/** Called to check if an asset should be filtered out by external code. Return true to exclude the asset from the view. */
	DECLARE_DELEGATE_RetVal_OneParam(bool, FOnShouldFilterAsset, const FOasisAssetData& /*AssetData*/);
...
// 
// 	/** Called to request the menu when right clicking on an asset */
 	DECLARE_DELEGATE_RetVal_OneParam(TSharedPtr<SWidget>, FOnGetExtAssetContextMenu, const TArray<FOasisAssetData>& /*SelectedAssets*/);

public:
	SLATE_BEGIN_ARGS(SOasisAssetView)
		...
		SLATE_ARGUMENT( FOasisAssetData, InitialAssetSelection )
	...
	void SyncTo( const FOasisContentBrowserSelection& ItemSelection, const bool bFocusOnSync = true );
	...
	TArray<FOasisAssetData> GetSelectedAssets() const;
	...
	virtual void OnAssetRegistryAssetGathered(const FOasisAssetData& AssetData, int32 Left);
...
private:
	...
	void OnAssetUpdated(const FOasisAssetData& AssetData);
	...
	bool PassesCurrentFrontendFilter(const FOasisAssetData& Item) const;

#if ECB_FEA_ASYNC_ASSET_DISCOVERY
	/** Returns true if the specified asset data item passes all applied backend (asset registry) filters */
	bool PassesCurrentBackendFilter(const FOasisAssetData& Item) const;

	/** Removes asset data items from the given array that don't pass all applied backend (asset registry) filters */
	void RunAssetsThroughBackendFilter(TArray<FOasisAssetData>& InOutAssetDataList) const;
#endif
	...
	TSharedPtr<class FOasisAssetThumbnail> AddItemToNewThumbnailRelevancyMap(const TSharedPtr<FExtAssetViewAsset>& Item, TMap< TSharedPtr<FExtAssetViewAsset>, TSharedPtr<class FOasisAssetThumbnail> >& NewRelevantThumbnails);
	...
	void OnAssetsOrPathsDragDroppedToContentBrowser(const TArray<FOasisAssetData>& AssetList, const TArray<FString>& AssetPaths, const FString& DestinationPath);

	/** Delegate to respond to drop of assets or asset paths onto a folder */
	void ExecuteImportAndPlaceSelecteUAssetFiles();

	/** Delegate to respond to drop of assets or asset paths onto a folder */
	void ExecuteDropToContentBrowserToImport(TArray<FOasisAssetData> AssetList, TArray<FString> AssetPaths, FString DestinationPath);

	/** Delegate to respond to drop of assets or asset paths onto a folder */
	void ExecuteDropToContentBrowserToImportFlat(TArray<FOasisAssetData> AssetList, TArray<FString> AssetPaths, FString DestinationPath);
#endif
...
private:
	...
	TArray<FOasisAssetData> QueriedAssetItems;
	TArray<FOasisAssetData> AssetItems;
	...
	TSet<FOasisAssetData> RecentlyLoadedOrChangedAssets;

	/** A list of assets that were recently reported as added by the asset registry */
	TArray<FOasisAssetData> RecentlyAddedAssets;
	TArray<FOasisAssetData> FilteredRecentlyAddedAssets;
	...
	TSharedPtr<class FOasisAssetThumbnailPool> AssetThumbnailPool;
	...
	TMap< TSharedPtr<FExtAssetViewAsset>, TSharedPtr<class FOasisAssetThumbnail> > RelevantThumbnails;
