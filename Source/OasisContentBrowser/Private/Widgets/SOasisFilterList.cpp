...
public:
	...
	SLATE_BEGIN_ARGS( SExtFilter ){}
		...
		SLATE_EVENT( SOasisFilterList::FOnFilterChanged, OnFilterChanged )
...
private:
	/** Invoked when the filter toggled */
	SOasisFilterList::FOnFilterChanged OnFilterChanged;
...
void SOasisFilterList::Construct( const FArguments& InArgs )
{
...
FReply SOasisFilterList::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
const TArray<UClass*>& SOasisFilterList::GetInitialClassFilters()
{
	return InitialClassFilters;
}

bool SOasisFilterList::HasAnyFilters() const
{
	return Filters.Num() > 0;
}

FARFilter SOasisFilterList::GetCombinedBackendFilter() const
{
...
TSharedRef<SWidget> SOasisFilterList::ExternalMakeAddFilterMenu(EAssetTypeCategories::Type MenuExpansion)
{
	return MakeAddFilterMenu(MenuExpansion);
}

void SOasisFilterList::EnableAllFilters()
{
...
void SOasisFilterList::DisableAllFilters()
{
...
void SOasisFilterList::RemoveAllFilters()
{
	if ( HasAnyFilters() )
	{
		for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt )
...
void SOasisFilterList::DisableFiltersThatHideAssets(const TArray<FOasisAssetData>& AssetDataList)
{
...
void SOasisFilterList::SaveSettings(const FString& IniFilename, const FString& IniSection, const FString& SettingsString) const
{
...
void SOasisFilterList::LoadSettings(const FString& IniFilename, const FString& IniSection, const FString& SettingsString)
{
	{
...
void SOasisFilterList::SetFrontendFilterActive(const TSharedRef<FExtFrontendFilter>& Filter, bool bActive)
{
...
TSharedRef<SExtFilter> SOasisFilterList::AddFilter(const TWeakPtr<IAssetTypeActions>& AssetTypeActions)
{
	TSharedRef<SExtFilter> NewFilter =
		SNew(SExtFilter)
		.AssetTypeActions(AssetTypeActions)
		.OnFilterChanged(OnFilterChanged)
		.OnRequestRemove(this, &SOasisFilterList::RemoveFilterAndUpdate)
		.OnRequestEnableOnly(this, &SOasisFilterList::EnableOnlyThisFilter)
		.OnRequestEnableAll(this, &SOasisFilterList::EnableAllFilters)
		.OnRequestDisableAll(this, &SOasisFilterList::DisableAllFilters)
		.OnRequestRemoveAll(this, &SOasisFilterList::RemoveAllFilters);
...
TSharedRef<SExtFilter> SOasisFilterList::AddFilter(const TSharedRef<FExtFrontendFilter>& FrontendFilter)
{
	TSharedRef<SExtFilter> NewFilter =
		SNew(SExtFilter)
		.FrontendFilter(FrontendFilter)
		.OnFilterChanged( this, &SOasisFilterList::FrontendFilterChanged, FrontendFilter )
		.OnRequestRemove(this, &SOasisFilterList::RemoveFilterAndUpdate)
		.OnRequestEnableOnly(this, &SOasisFilterList::EnableOnlyThisFilter)
		.OnRequestEnableAll(this, &SOasisFilterList::EnableAllFilters)
		.OnRequestDisableAll(this, &SOasisFilterList::DisableAllFilters)
...
void SOasisFilterList::AddFilter(const TSharedRef<SExtFilter>& FilterToAdd)
{
...
void SOasisFilterList::RemoveFilter(const TWeakPtr<IAssetTypeActions>& AssetTypeActions, bool ExecuteOnFilterChanged)
{
...
void SOasisFilterList::EnableOnlyThisFilter(const TSharedRef<SExtFilter>& FilterToEnable)
{
	for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt )
	{
...
void SOasisFilterList::RemoveFilter(const TSharedRef<FExtFrontendFilter>& FrontendFilter, bool ExecuteOnFilterChanged)
{
	TSharedPtr<SExtFilter> FilterToRemove;
	for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt )
	{
...
void SOasisFilterList::RemoveFilter(const TSharedRef<SExtFilter>& FilterToRemove)
{
...
void SOasisFilterList::RemoveFilterAndUpdate(const TSharedRef<SExtFilter>& FilterToRemove)
{
...
void SOasisFilterList::FrontendFilterChanged(TSharedRef<FExtFrontendFilter> FrontendFilter)
{
...
void SOasisFilterList::CreateFiltersMenuCategory(FMenuBuilder& MenuBuilder, const TArray<TWeakPtr<IAssetTypeActions>> AssetTypeActionsList) const
{
		...
		if ( WeakTypeActions.IsValid() )
		{
			...
			if ( TypeActions.IsValid() )
			{
				...
				MenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateSP( const_cast<SOasisFilterList*>(this), &SOasisFilterList::FilterByTypeClicked, WeakTypeActions ),
						FCanExecuteAction(),
						FIsActionChecked::CreateSP(this, &SOasisFilterList::IsAssetTypeActionsInUse, WeakTypeActions ) ),
					NAME_None,
...
void SOasisFilterList::CreateOtherFiltersMenuCategory(FMenuBuilder& MenuBuilder, TSharedPtr<FExtFrontendFilterCategory> MenuCategory) const
{	
	for ( auto FrontendFilterIt = AllFrontendFilters.CreateConstIterator(); FrontendFilterIt; ++FrontendFilterIt )
	{
		...
		if(FrontendFilter->GetCategory() == MenuCategory)
		{
			MenuBuilder.AddMenuEntry(
				...
				FUIAction(
				FExecuteAction::CreateSP( const_cast<SOasisFilterList*>(this), &SOasisFilterList::FrontendFilterClicked, FrontendFilter ),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SOasisFilterList::IsFrontendFilterInUse, FrontendFilter ) ),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
				);
		}
...
TSharedRef<SWidget> SOasisFilterList::MakeAddFilterMenu(EAssetTypeCategories::Type MenuExpansion)
{
	...
	MenuBuilder.BeginSection( "ContentBrowserResetFilters" );
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction( FExecuteAction::CreateSP( this, &SOasisFilterList::OnResetFilters ) )
			);
	}
	...
	MenuBuilder.BeginSection(ExpandedCategory->SectionExtensionHook, ExpandedCategory->SectionHeading );
	{
		if(MenuExpansion == EAssetTypeCategories::Basic)
		{
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
				FExecuteAction::CreateSP( this, &SOasisFilterList::FilterByTypeCategoryClicked, MenuExpansion ),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SOasisFilterList::IsAssetTypeCategoryInUse, MenuExpansion ) ),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
				);
		}
		...
		SOasisFilterList::CreateFiltersMenuCategory( MenuBuilder, ExpandedCategory->Assets);
	}
	...
	MenuBuilder.BeginSection("ContentBrowserFilterAdvancedAsset", LOCTEXT("AdvancedAssetsMenuHeading", "Other Assets") );
	{
		if(MenuExpansion == EAssetTypeCategories::Basic)
		{
			...
			for ( auto MenuIt = CategoryToMenuMap.CreateIterator(); MenuIt; ++MenuIt )
			{
				...
				MenuBuilder.AddSubMenu(
					Menu.Name,
					Menu.Tooltip,
					FNewMenuDelegate::CreateSP(this, &SOasisFilterList::CreateFiltersMenuCategory, Menu.Assets),
					FUIAction(
					FExecuteAction::CreateSP( this, &SOasisFilterList::FilterByTypeCategoryClicked, MenuIt.Key() ),
					FCanExecuteAction(),
					FIsActionChecked::CreateSP(this, &SOasisFilterList::IsAssetTypeCategoryInUse, MenuIt.Key() ) ),
					NAME_None,
					EUserInterfaceActionType::ToggleButton
					);
			}
		...
		for(auto Iter = AllFrontendFilterCategories.CreateIterator(); Iter; ++Iter)
		{
			...
			MenuBuilder.AddSubMenu(
				Category->Title,
				Category->Tooltip,
				FNewMenuDelegate::CreateSP(this, &SOasisFilterList::CreateOtherFiltersMenuCategory, Category),
				FUIAction(
				FExecuteAction::CreateSP( this, &SOasisFilterList::FrontendFilterCategoryClicked, Category ),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SOasisFilterList::IsFrontendFilterCategoryInUse, Category ) ),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
				);
		}
...
void SOasisFilterList::FilterByTypeClicked(TWeakPtr<IAssetTypeActions> AssetTypeActions)
{
	if ( AssetTypeActions.IsValid() )
	{
...
bool SOasisFilterList::IsAssetTypeActionsInUse(TWeakPtr<IAssetTypeActions> AssetTypeActions) const
{
	if ( !AssetTypeActions.IsValid() )
	{
		return false;
...
void SOasisFilterList::FilterByTypeCategoryClicked(EAssetTypeCategories::Type Category)
{
...
bool SOasisFilterList::IsAssetTypeCategoryInUse(EAssetTypeCategories::Type Category) const
{
	TArray<TWeakPtr<IAssetTypeActions>> TypeActionsList;
	GetTypeActionsForCategory(Category, TypeActionsList);
...
void SOasisFilterList::GetTypeActionsForCategory(EAssetTypeCategories::Type Category, TArray< TWeakPtr<IAssetTypeActions> >& TypeActions) const
{
...
void SOasisFilterList::FrontendFilterClicked(TSharedRef<FExtFrontendFilter> FrontendFilter)
{
	if ( IsFrontendFilterInUse(FrontendFilter) )
	{
...
bool SOasisFilterList::IsFrontendFilterInUse(TSharedRef<FExtFrontendFilter> FrontendFilter) const
{
	for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt)
	{
		TSharedPtr<FExtFrontendFilter> Filter = (*FilterIt)->GetFrontendFilter();
...
void SOasisFilterList::FrontendFilterCategoryClicked(TSharedPtr<FExtFrontendFilterCategory> MenuCategory)
{
...
bool SOasisFilterList::IsFrontendFilterCategoryInUse(TSharedPtr<FExtFrontendFilterCategory> MenuCategory) const
{
	for ( auto FrontendFilterIt = AllFrontendFilters.CreateConstIterator(); FrontendFilterIt; ++FrontendFilterIt )
	{
		const TSharedRef<FExtFrontendFilter>& FrontendFilter = *FrontendFilterIt;
...
void SOasisFilterList::OnResetFilters()
{
	RemoveAllFilters();
}
