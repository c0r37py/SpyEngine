 ...
 */
class SOasisPathView : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SOasisPathView )
	 ...
	 */
	void SyncToAssets( const TArray<FOasisAssetData>& AssetDataList, const bool bAllowImplicitSync = false );
	 ...
	 *								  false to force the view to select the explicit Parent folders of each asset 
	 */
	void SyncTo( const FOasisContentBrowserSelection& ItemSelection, const bool bAllowImplicitSync = false );
...
private:
	...
	void SyncToInternal( const TArray<FOasisAssetData>& AssetDataList, const TArray<FString>& FolderPaths, const bool bAllowImplicitSync );
...
protected:
	...
	{
	public:
		FScopedPreventTreeItemChangedDelegate(const TSharedRef<SOasisPathView>& InPathView)
			: PathView(InPathView)
		{
	...
	private:
		TSharedRef<SOasisPathView> PathView;
	};
...
*/
class SFavoritePathView : public SOasisPathView
{
public:
	/** Constructs this widget with InArgs */
