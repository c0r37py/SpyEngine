...
void SExtAssetTreeItem::Construct( const FArguments& InArgs )
{
	...
	FolderClosedBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderClosed");
	FolderOpenBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderOpen");
	
	FolderClosedCodeBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderClosedCode");
	FolderOpenCodeBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderOpenCode");
	
	FolderDeveloperBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderDeveloper");

	FolderClosedPluginBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderClosedPlugin");
	FolderOpenPluginBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderOpenPlugin");
	FolderClosedProjectBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderClosedProject");
	FolderOpenProjectBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderOpenProject");
	FolderClosedVaultCacheBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderClosedVaultCache");
	FolderOpenVaultCacheBrush = FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.AssetTreeFolderOpenVaultCache");
		
	FolderType = EFolderType::Normal;

#if ECB_DISABLE
	if( OasisContentBrowserUtils::IsDevelopersFolder(InArgs._TreeItem->FolderPath) )
	{
		FolderType = EFolderType::Developer;
	}
	else if( OasisContentBrowserUtils::IsClassPath/*IsClassRootDir*/(InArgs._TreeItem->FolderPath) )
	{
	...
	if (bIsRoot)
	{
		if (OasisContentBrowserUtils::ECBFolderCategory::ProjectContent == OasisContentBrowserUtils::GetRootFolderCategory(InArgs._TreeItem->FolderPath))
		{
			FolderType = EFolderType::Project;
		}
		else if (OasisContentBrowserUtils::ECBFolderCategory::VaultCacheContent == OasisContentBrowserUtils::GetRootFolderCategory(InArgs._TreeItem->FolderPath))
		{
			FolderType = EFolderType::VaultCache;
		}
		else if (OasisContentBrowserUtils::ECBFolderCategory::PluginContent == OasisContentBrowserUtils::GetRootFolderCategory(InArgs._TreeItem->FolderPath))
		{
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(SSpinningImage)
					.Image(FOasisContentBrowserStyle::Get().GetBrush("SpyEngine.Rotation16px"))
			...
			[
				SAssignNew(InlineRenameWidget, SInlineEditableTextBlock)
					...
					.Font( InArgs._FontOverride.IsSet() ? InArgs._FontOverride : FOasisContentBrowserStyle::GetFontStyle(bIsRoot ? "SpyEngine.SourceTreeRootItemFont" : "SpyEngine.SourceTreeItemFont") )
			...
			[
				SNew(STextBlock)
				.TextStyle(FOasisContentBrowserStyle::Get(), "SpyEngine.SourceTreeRootItem.Loading")
...
bool SExtAssetTreeItem::IsValidAssetPath() const
{
	if ( TreeItem.IsValid() )
	{
		...
		return !OasisContentBrowserUtils::IsClassPath(TreeItem.Pin()->FolderPath);
	}
	else
	{
		return false;
...
FSlateColor SExtAssetTreeItem::GetFolderColor() const
{
	...
	{
#if ECB_LEGACY
		const TSharedPtr<FLinearColor> Color = OasisContentBrowserUtils::LoadColor( TreeItem.Pin()->FolderPath );
		if ( Color.IsValid() )
		{
			return *Color.Get();
		}
		...
		if (OasisContentBrowserUtils::LoadFolderColor(TreeItem.Pin()->FolderPath, FolderColor))
		{
			return FolderColor;
		}
	}
