...
class SOasisAssetViewItemToolTip : public SToolTip
{
public:
	SLATE_BEGIN_ARGS(SOasisAssetViewItemToolTip)
		: _AssetViewItem()
	{ }

		SLATE_ARGUMENT(TSharedPtr<SOasisAssetViewItem>, AssetViewItem)
	...
	{
		TSharedPtr<SOasisAssetViewItem> AssetViewItemPin = AssetViewItem.Pin();
...
private:
	TWeakPtr<SOasisAssetViewItem> AssetViewItem;
};
...
SOasisAssetViewItem::~SOasisAssetViewItem()
{
...
void SOasisAssetViewItem::Construct( const FArguments& InArgs )
{
	...
	AssetItem->OnAssetDataChanged.AddSP(this, &SOasisAssetViewItem::OnAssetDataChanged);
	...
	SetToolTip(SNew(SOasisAssetViewItemToolTip).AssetViewItem(SharedThis(this)));

	SourceControlStateDelay = 0.0f;
	bSourceControlStateRequested = false;

	ISourceControlModule::Get().RegisterProviderChanged(FSourceControlProviderChanged::FDelegate::CreateSP(this, &SOasisAssetViewItem::HandleSourceControlProviderChanged));
	SourceControlStateChangedDelegateHandle = ISourceControlModule::Get().GetProvider().RegisterSourceControlStateChanged_Handle(FSourceControlStateChanged::FDelegate::CreateSP(this, &SOasisAssetViewItem::HandleSourceControlStateChanged));
...
void SOasisAssetViewItem::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
TSharedPtr<IToolTip> SOasisAssetViewItem::GetToolTip()
{
	return ShouldAllowToolTip.Get() ? SCompoundWidget::GetToolTip() : NULL;
}

bool SOasisAssetViewItem::ValidateDragDrop( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent, bool& OutIsKnownDragOperation ) const
{
...
void SOasisAssetViewItem::OnDragEnter( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver); // updates bDraggedOver
}
	
void SOasisAssetViewItem::OnDragLeave( const FDragDropEvent& DragDropEvent )
{
	if(IsFolder())
	{
...
FReply SOasisAssetViewItem::OnDragOver( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver); // updates bDraggedOver
	return (bDraggedOver) ? FReply::Handled() : FReply::Unhandled();
}

FReply SOasisAssetViewItem::OnDrop( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
...
bool SOasisAssetViewItem::IsNameReadOnly() const
{
	return true;
}

void SOasisAssetViewItem::HandleBeginNameChange( const FText& OriginalText )
{
	OnRenameBegin.ExecuteIfBound(AssetItem, OriginalText.ToString(), LastGeometry.GetLayoutBoundingRect());
}

void SOasisAssetViewItem::HandleNameCommitted( const FText& NewText, ETextCommit::Type CommitInfo )
{
	OnRenameCommit.ExecuteIfBound(AssetItem, NewText.ToString(), LastGeometry.GetLayoutBoundingRect(), CommitInfo);
}

bool  SOasisAssetViewItem::HandleVerifyNameChanged( const FText& NewText, FText& OutErrorMessage )
{
...
void SOasisAssetViewItem::OnAssetDataChanged()
{
	CachePackageName();
...
void SOasisAssetViewItem::DirtyStateChanged()
{
}

FText SOasisAssetViewItem::GetAssetClassText() const
{
	if (!AssetItem.IsValid())
	{
		return FText();
...
void SOasisAssetViewItem::HandleSourceControlProviderChanged(ISourceControlProvider& OldProvider, ISourceControlProvider& NewProvider)
{
	OldProvider.UnregisterSourceControlStateChanged_Handle(SourceControlStateChangedDelegateHandle);
	SourceControlStateChangedDelegateHandle = NewProvider.RegisterSourceControlStateChanged_Handle(FSourceControlStateChanged::FDelegate::CreateSP(this, &SOasisAssetViewItem::HandleSourceControlStateChanged));
...
void SOasisAssetViewItem::HandleSourceControlStateChanged()
{
...
const FSlateBrush* SOasisAssetViewItem::GetDirtyImage() const
{
	return IsDirty() ? AssetDirtyBrush : NULL;
}


TSharedRef<SWidget> SOasisAssetViewItem::GenerateSourceControlIconWidget()
{
...
TSharedRef<SWidget> SOasisAssetViewItem::GenerateExtraStateIconWidget(TAttribute<float> InMaxExtraStateIconWidth) const
{
#if ECB_TODO // Extra icon support
	TArray<FOnGenerateAssetViewExtraStateIndicators>& ExtraStateIconGenerators = FModuleManager::GetModuleChecked<FOasisContentBrowserModule>(TEXT("OasisContentBrowser")).GetAllAssetViewExtraStateIconGenerators();
...
TSharedRef<SWidget> SOasisAssetViewItem::GenerateExtraStateTooltipWidget() const
{
#if ECB_TODO // Extra icon support
	TArray<FOnGenerateAssetViewExtraStateIndicators>& ExtraStateTooltipGenerators = FModuleManager::GetModuleChecked<FOasisContentBrowserModule>(TEXT("ContentBrowser")).GetAllAssetViewExtraStateTooltipGenerators();
...
EVisibility SOasisAssetViewItem::GetThumbnailEditModeUIVisibility() const
{
	return !IsFolder() && ThumbnailEditMode.Get() ? EVisibility::Visible : EVisibility::Collapsed;
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedRef<SWidget> SOasisAssetViewItem::CreateToolTipWidget() const
{
	if ( AssetItem.IsValid() )
	{
		...
		{
			FOasisAssetData& AssetData = StaticCastSharedPtr<FAssetViewAsset>(AssetItem)->Data;
		...
		if(AssetItem->GetType() != EExtAssetItemType::Folder)
		{
			const FOasisAssetData& AssetData = StaticCastSharedPtr<FExtAssetViewAsset>(AssetItem)->Data;
...
#if ECB_WIP_COLLECTION
			{
				...
				const FString CollectionNames = CollectionManagerModule.Get().GetCollectionsStringForObject(AssetData.ObjectPath, ECollectionShareType::CST_All, ECollectionRecursionFlags::Self, GetDefault<UOasisContentBrowserSettings>()->bShowFullCollectionNameInToolTip);
			...
			TOptional<FAssetImportInfo> ImportInfo = FOasisContentBrowserSingleton::GetAssetRegistry().ExtractAssetImportInfo(AssetData);
			if (ImportInfo.IsSet())
			{
...
FText SOasisAssetViewItem::GetAssetUserDescription() const
{
...
void SOasisAssetViewItem::AddToToolTipInfoBox(const TSharedRef<SVerticalBox>& InfoBox, const FText& Key, const FText& Value, bool bImportant) const
{
...
void SOasisAssetViewItem::UpdatePackageDirtyState()
{
...
bool SOasisAssetViewItem::IsDirty() const
{
	return bPackageDirty;
}

void SOasisAssetViewItem::UpdateSourceControlState(float InDeltaTime)
{
...
void SOasisAssetViewItem::CachePackageName()
{
#if ECB_LEGACY
	if(AssetItem.IsValid())
	{
...
void SOasisAssetViewItem::CacheDisplayTags()
{
		...
		// Add the type and display flags
		{
			...
			else
			{
				PRAGMA_DISABLE_DEPRECATION_WARNINGS
					AssetCDO->GetAssetRegistryTags(TmpTags); // Use deprecated method temporarily
...
FSlateColor SOasisAssetViewItem::GetAssetColor() const
{
	if(AssetItem.IsValid())
	{
		if(AssetItem->GetType() == EExtAssetItemType::Folder)
		{
			...
			{
#if ECB_LEGACY
				Color = OasisContentBrowserUtils::LoadColor( AssetFolderItem->FolderPath );
#endif
			}
#if ECB_LEGACY
			if ( Color.IsValid() )
			...
			if (OasisContentBrowserUtils::LoadFolderColor(AssetFolderItem->FolderPath, FolderColor))
			{
				return FolderColor;
			}
			else
			{
				return OasisContentBrowserUtils::GetDefaultColor();
			}
		}
...
void SOasisAssetViewItem::SetForceMipLevelsToBeResident(bool bForce) const
{
#if ECB_LEGACY
	if(AssetItem.IsValid() && AssetItem->GetType() == EAssetItemType::Normal)
	{
		const FOasisAssetData& AssetData = StaticCastSharedPtr<FAssetViewAsset>(AssetItem)->Data;
...
bool SOasisAssetViewItem::OnVisualizeTooltip(const TSharedPtr<SWidget>& TooltipContent)
{
#if ECB_LEGACY
...
void SOasisAssetViewItem::OnToolTipClosing()
{
...
{
	SOasisAssetViewItem::Construct( SOasisAssetViewItem::FArguments()
		.AssetItem(InArgs._AssetItem)
		.OnRenameBegin(InArgs._OnRenameBegin)
		.OnRenameCommit(InArgs._OnRenameCommit)
		.OnVerifyRenameCommit(InArgs._OnVerifyRenameCommit)
	...
	[
		SNew(SBorder)
		.BorderImage(this, &SOasisAssetViewItem::GetBorderImage)
		.Padding(0)
...
{
	SOasisAssetViewItem::OnAssetDataChanged();
...
private:
	TWeakPtr<SOasisAssetViewItem> OwnerAssetColumnItem;
};
