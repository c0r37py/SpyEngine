...
class SOasisAssetView;
class SCollectionView;
class SComboButton;
class SOasisFilterList;
class SOasisPathView;
class SSplitter;
class UFactory;
...
public:
	...
	SLATE_BEGIN_ARGS(SOasisContentBrowser)
 		: _ContainingTab()
	 ...
	 */
	void SyncToAssets( const TArray<FOasisAssetData>& AssetDataList, const bool bAllowImplicitSync = false, const bool bDisableFiltersThatHideAssets = true );
	 ...
	 *								  false to force the view to select the explicit Parent folders of each asset 
	 */
	void SyncTo( const FOasisContentBrowserSelection& ItemSelection, const bool bAllowImplicitSync = false, const bool bDisableFiltersThatHideAssets = true );
	...
	void GetSelectedAssets(TArray<FOasisAssetData>& SelectedAssets);
...
private:
	...
	void ImportAsset(const TArray<FOasisAssetData>& InAssetDatas);

	/** Imports uasset files to sandbox folder. */
	void ImportAssetToSandbox(const TArray<FOasisAssetData>& InAssetDatas);

	void ImportAssetFlatMode(const TArray<FOasisAssetData>& InAssetDatas);
	...
	void OnAssetSelectionChanged(const FOasisAssetData& SelectedAsset);
#if ECB_LEGACY
	...
	TSharedPtr<SWidget> OnGetAssetContextMenu(const TArray<FOasisAssetData>& SelectedAssets);
	...
	void GetSelectionState(TArray<FOasisAssetData>& SelectedAssets, TArray<FString>& SelectedPaths);
#endif
...
private:
	...
	TSharedPtr<SOasisPathView> PathViewPtr;
	...
	TSharedPtr<SOasisAssetView> AssetViewPtr;
...
#endif
	/** The filter list */
	TSharedPtr<SOasisFilterList> FilterListPtr;

	/** The filter list */
	TSharedPtr<SOasisDependencyViewer> ReferenceViewerPtr;
