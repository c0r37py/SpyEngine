...
bool FExtUDNParser::GetExcerptContent( const FString& Link, FExcerpt& Excerpt, bool bInSimpleText/* = false*/ )
{
	...
	else
	{
		...
		UDNParserLog.Error(FText::Format(LOCTEXT("GeneralExcerptError", "Generating a Widget for document '{0}' Excerpt '{1}' failed."), FText::FromString( FOasisDocumentationLink::ToSourcePath( Link ) ), FText::FromString(Excerpt.Name) ));
	}
...
void FExtUDNParser::AddContentToExcerpt(TSharedPtr<SVerticalBox> Box, const FString& ContentSource, FExcerpt& Excerpt)
{
	...
	{
		const ISlateStyle& DocumentationStyle = FOasisDocumentationStyle::Get();
...
TSharedRef< SWidget > FExtUDNParser::GenerateExcerptContent( const FString& InLink, FExcerpt& Excerpt, const TArray<FString>& ContentLines, int32 StartingLineIndex )
{
	...
	const FString SourcePath = FOasisDocumentationLink::ToSourcePath( InLink );
			...
			case FUDNLine::HorizontalRule:
				...
				Box->AddSlot()
					...
					[
						...
						[
							SNew(SSeparator)
							.SeparatorImage(FOasisDocumentationStyle::Get().GetBrush(Style.SeparatorStyleName))
						]
					];
