...
void SOasisDocumentationToolTip::Construct( const FArguments& InArgs )
{
...
void SOasisDocumentationToolTip::ConstructSimpleTipContent()
{
...
void SOasisDocumentationToolTip::AddDocumentation(TSharedPtr< SVerticalBox > VerticalBox)
{
	if ( !DocumentationLink.IsEmpty() )
	{
		...
		else
		{
			...
			{
				FString DocPath = FOasisDocumentationLink::ToSourcePath( DocumentationLink, FInternationalization::Get().GetCurrentCulture() );
				...
				[
					...
					.OnNavigate( this, &SOasisDocumentationToolTip::CreateExcerpt, DocPath, ExcerptName )
				];
			}
		}
	}
...
void SOasisDocumentationToolTip::CreateExcerpt( FString FileSource, FString InExcerptName )
{
...
void SOasisDocumentationToolTip::ConstructFullTipContent()
{
			...
			if ( GetDefault<UEditorPerProjectUserSettings>()->bDisplayDocumentationLink && FSlateApplication::Get().SupportsSourceAccess() )
			{
				...
				[
					SNew( SHyperlink )
						...
						.TextStyle( &HyperlinkTextStyleInfo )
						.UnderlineStyle( &HyperlinkButtonStyleInfo )
						.OnNavigate_Static(&Local::EditSource, FPaths::ConvertRelativePathToFull(FOasisDocumentationLink::ToSourcePath(DocumentationLink, FInternationalization::Get().GetCurrentCulture())), Excerpts[ExcerptIndex].LineNumber)
				];
			}
		}
	}
...
FReply SOasisDocumentationToolTip::ReloadDocumentation()
{
...
void SOasisDocumentationToolTip::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
bool SOasisDocumentationToolTip::IsInteractive() const
{
	const FModifierKeysState ModifierKeys = FSlateApplication::Get().GetModifierKeys();
