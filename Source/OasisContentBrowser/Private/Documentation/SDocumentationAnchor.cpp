...
void SOasisDocumentationAnchor::Construct(const FArguments& InArgs )
{
	Link = InArgs._Link;
	...
	[
		...
		.OnClicked( this, &SOasisDocumentationAnchor::OnClicked )
		...
		[
			...
			.Image( this, &SOasisDocumentationAnchor::GetButtonImage )
		]
	];
}

const FSlateBrush* SOasisDocumentationAnchor::GetButtonImage() const
{
	if ( Button->IsPressed() )
	{
		return Pressed;
...
FReply SOasisDocumentationAnchor::OnClicked() const
{
