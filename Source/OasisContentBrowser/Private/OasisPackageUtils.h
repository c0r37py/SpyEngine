 ...
 */
struct FOasisPackageUtils
{
public:

	/** Parse and gather external asset data */
	static bool ParsePackageWithPackageReader(FOasisAssetData& OutOasisAssetData);

	/** Load thumbnail from external asset */
	static bool LoadThumbnailWithPackageReader(const FOasisAssetData& InOasisAssetData, FObjectThumbnail& InOutThumbnail);

#if ECB_LEGACY

	/** Parse and gather external asset data */
	static bool ParsePackage(FOasisAssetData& OutOasisAssetData);

	/** Load thumbnail from external asset */
	static bool LoadThumbnail(const FOasisAssetData& InOasisAssetData, FObjectThumbnail& OutThumbnail);

	/** Load soft references */
	static void LoadSoftReferences(const FOasisAssetData& InOasisAssetData, TArray<FName>& OutSoftPackageReferenceList);
