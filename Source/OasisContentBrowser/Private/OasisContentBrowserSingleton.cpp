...
/////////////////////////////////////////////////////////////
// FOasisContentBrowserSingleton implementation
//

FExtAssetRegistry FOasisContentBrowserSingleton::ExtAssetRegistry;

FOasisContentBrowserSingleton::FOasisContentBrowserSingleton()
// 	, SettingsStringID(0)
{
	...
	if (OasisContentBrowserSetting->bUseThumbnailPool && OasisContentBrowserSetting->NumThumbnailsInPool > 0)
	{
		ThumbnailPool.Reserve(OasisContentBrowserSetting->NumThumbnailsInPool); // todo: move to setting
	}
#endif

	// Style
	FOasisContentBrowserStyle::Initialize();
	FOasisContentBrowserStyle::ReloadTextures();
...

	{
		...
		OasisContentBrowserSetting->DisplayToolbarButton = false;
...
FOasisContentBrowserSingleton::~FOasisContentBrowserSingleton()
{
	...
	FOasisContentBrowserStyle::Shutdown();

	FOasisContentBrowserCommands::Unregister();
...
void FContentBrowserSingleton::FocusPrimaryContentBrowser(bool bFocusSearch)
{
	...
	if ( PrimaryContentBrowser.IsValid() )
	{
		FocuSOasisContentBrowser( PrimaryContentBrowser.Pin() );
	}
	else
	{
...
TSharedPtr<SOasisContentBrowser> FContentBrowserSingleton::FindContentBrowserToSync(bool bAllowLockedBrowsers, const FName& InstanceName, bool bNewSpawnBrowser)
{
	TSharedPtr<SOasisContentBrowser> ContentBrowserToSync;
...
void FContentBrowserSingleton::SyncBrowserToAssets(const TArray<FAssetData>& AssetDataList, bool bAllowLockedBrowsers, bool bFocuSOasisContentBrowser, const FName& InstanceName, bool bNewSpawnBrowser)
{
	TSharedPtr<SOasisContentBrowser> ContentBrowserToSync = FindContentBrowserToSync(bAllowLockedBrowsers, InstanceName, bNewSpawnBrowser);

	if ( ContentBrowserToSync.IsValid() )
	{
		// Finally, focus and sync the browser that was found
		if (bFocuSOasisContentBrowser)
		{
			FocuSOasisContentBrowser(ContentBrowserToSync);
		}
		ContentBrowserToSync->SyncToAssets(AssetDataList);
	}
}

void FContentBrowserSingleton::SyncBrowserToAssets(const TArray<UObject*>& AssetList, bool bAllowLockedBrowsers, bool bFocuSOasisContentBrowser, const FName& InstanceName, bool bNewSpawnBrowser)
{
	...
	SyncBrowserToAssets(AssetDataList, bAllowLockedBrowsers, bFocuSOasisContentBrowser, InstanceName, bNewSpawnBrowser);
}

void FContentBrowserSingleton::SyncBrowserToFolders(const TArray<FString>& FolderList, bool bAllowLockedBrowsers, bool bFocuSOasisContentBrowser, const FName& InstanceName, bool bNewSpawnBrowser)
{
	TSharedPtr<SOasisContentBrowser> ContentBrowserToSync = FindContentBrowserToSync(bAllowLockedBrowsers, InstanceName, bNewSpawnBrowser);
...
void FContentBrowserSingleton::SyncBrowserTo(const FOasisContentBrowserSelection& ItemSelection, bool bAllowLockedBrowsers, bool bFocuSOasisContentBrowser, const FName& InstanceName, bool bNewSpawnBrowser)
{
	TSharedPtr<SOasisContentBrowser> ContentBrowserToSync = FindContentBrowserToSync(bAllowLockedBrowsers, InstanceName, bNewSpawnBrowser);
...
void FOasisContentBrowserSingleton::GetSelectedAssets(TArray<FOasisAssetData>& SelectedAssets)
{
	if ( PrimaryContentBrowser.IsValid() )
	{
...
FOasisContentBrowserSingleton& FOasisContentBrowserSingleton::Get()
{
	static const FName ModuleName = "OasisContentBrowser";
	FOasisContentBrowserModule& Module = FModuleManager::GetModuleChecked<FOasisContentBrowserModule>(ModuleName);
	return static_cast<FOasisContentBrowserSingleton&>(Module.Get());
}

FExtAssetRegistry& FOasisContentBrowserSingleton::GetAssetRegistry()
{
	return ExtAssetRegistry;
}

FExtObjectThumbnailPool& FOasisContentBrowserSingleton::GetThumbnailPool()
{
	return Get().ThumbnailPool;
}

FString FOasisContentBrowserSingleton::GetPluginResourcesDir()
{
	return Get().PluginInfo.ResourcesDir;
}

FText FOasisContentBrowserSingleton::GetPluginVersionText()
{
	return Get().PluginInfo.VersionName;
}

void FOasisContentBrowserSingleton::SetPrimaryContentBrowser(const TSharedRef<SOasisContentBrowser>& NewPrimaryBrowser)
{
...
void FContentBrowserSingleton::ContentBrowserClosed(const TSharedRef<SOasisContentBrowser>& ClosedBrowser)
{
...
void FContentBrowserSingleton::FocuSOasisContentBrowser(const TSharedPtr<SOasisContentBrowser>& BrowserToFocus)
{
	if ( BrowserToFocus.IsValid() )
	{
		TSharedRef<SOasisContentBrowser> Browser = BrowserToFocus.ToSharedRef();
...
FName FContentBrowserSingleton::SummonNewBrowser(bool bAllowLockedBrowsers)
{
	...
	for (int32 BrowserIdx = AllContentBrowsers.Num() - 1; BrowserIdx >= 0; --BrowserIdx)
	{
		const TWeakPtr<SOasisContentBrowser>& Browser = AllContentBrowsers[BrowserIdx];
		if ( Browser.IsValid() )
		{
...
#endif
TSharedRef<SWidget> FOasisContentBrowserSingleton::CreateContentBrowser( const FName InstanceName, TSharedPtr<SDockTab> ContainingTab)
{
	TSharedRef<SOasisContentBrowser> NewBrowser =
		SNew(SOasisContentBrowser, InstanceName)
		.IsEnabled( FSlateApplication::Get().GetNormalExecutionAttribute() )
...
void FOasisContentBrowserSingleton::RegisterEditorDelegates()
{
	FEditorDelegates::BeginPIE.AddRaw(this, &FOasisContentBrowserSingleton::OnBeginPIE);
	FEditorDelegates::EndPIE.AddRaw(this, &FOasisContentBrowserSingleton::OnEndPIE);
}

void FOasisContentBrowserSingleton::UnregisterEditorDelegates()
{
...
void FOasisContentBrowserSingleton::OnBeginPIE(const bool bIsSimulating)
{
}

void FOasisContentBrowserSingleton::OnEndPIE(const bool bIsSimulating)
{
}

void FOasisContentBrowserSingleton::MountSandbox()
{
#if ECB_WIP_IMPORT_SANDBOX
	FPackageName::RegisterMountPoint(FOasisAssetData::GetSandboxPackagePath() + TEXT("/"), FOasisAssetData::GetSandboxDir());
#endif
}

void FOasisContentBrowserSingleton::UnMountSandbox()
{
#if ECB_WIP_IMPORT_SANDBOX
	FPackageName::UnRegisterMountPoint(FOasisAssetData::GetSandboxPackagePath() + TEXT("/"), FOasisAssetData::GetSandboxDir());
#endif
}

void FOasisContentBrowserSingleton::RegisterMenuAndTabSpawner()
{
	...
	const FSlateIcon AssetBrowserIcon(FOasisContentBrowserStyle::GetStyleSetName(), "SpyEngine.Icon16x");

	for (int32 BrowserIdx = 0; BrowserIdx < UE_ARRAY_COUNT(ContentBrowserTabIDs); BrowserIdx++)
	{
		const FName TabID = FName(*FString::Printf(TEXT("SpyEngineBrowserTab%d"), BrowserIdx + 1));
		ContentBrowserTabIDs[BrowserIdx] = TabID;

		const FText DefaultDisplayName = GetContentBrowserLabelWithIndex(BrowserIdx);

		FGlobalTabmanager::Get()->RegisterNomadTabSpawner(TabID, FOnSpawnTab::CreateRaw(this, &FOasisContentBrowserSingleton::SpawnContentBrowserTab, BrowserIdx))
#if ECB_WIP_MULTI_INSTANCES
			.SetDisplayName(DefaultDisplayName)
#else
			.SetDisplayName(LOCTEXT("SpyEngineBrowserTabTitle", "SpyEngine"))
#endif
			.SetTooltipText(LOCTEXT("SpyEngineBrowserMenuTooltipText", "Open a SpyEngine tab."))
...
void FOasisContentBrowserSingleton::RegisterCommands()
{
	FOasisContentBrowserCommands::Register();

	OasisContentBrowserCommands = MakeShareable(new FUICommandList);

	OasisContentBrowserCommands->MapAction(FOasisContentBrowserCommands::Get().OpenSpyEngine, FExecuteAction::CreateRaw(this, &FOasisContentBrowserSingleton::OpenSpyEngineButtonClicked), FCanExecuteAction());
}

void FOasisContentBrowserSingleton::AddToolbarExtension(FToolBarBuilder& Builder)
{
	Builder.AddToolBarButton(FOasisContentBrowserCommands::Get().OpenSpyEngine);
}

void FOasisContentBrowserSingleton::OpenSpyEngineButtonClicked() const
{
	const FName OasisContentBrowserTabName("SpyEngineTab1");
	//FGlobalTabmanager::Get()->InvokeTab(OasisContentBrowserTabName);
	FLevelEditorModule& LevelEditorModule = FModuleManager::GetModuleChecked<FLevelEditorModule>(TEXT("LevelEditor"));
	LevelEditorModule.GetLevelEditorTabManager()->TryInvokeTab(OasisContentBrowserTabName);
}

TSharedRef<SDockTab> FOasisContentBrowserSingleton::SpawnContentBrowserTab( const FSpawnTabArgs& SpawnTabArgs, int32 BrowserIdx )
{	
	TAttribute<FText> Label = TAttribute<FText>::Create( TAttribute<FText>::FGetter::CreateRaw( this, &FOasisContentBrowserSingleton::GetContentBrowserTabLabel, BrowserIdx ) );
...
bool FContentBrowserSingleton::IsLocked(const FName& InstanceName) const
{
	...
	{
		const TWeakPtr<SOasisContentBrowser>& Browser = AllContentBrowsers[AllBrowsersIdx];
	...
	GConfig->GetBool(*SOasisContentBrowser::SettingsIniSection, *(InstanceName.ToString() + TEXT(".Locked")), bIsLocked, GEditorPerProjectIni);
	return bIsLocked;
}
#endif

FText FOasisContentBrowserSingleton::GetContentBrowserLabelWithIndex( int32 BrowserIdx )
{
	return FText::Format(LOCTEXT("SpyEngineTabNameWithIndex", "SpyEngine {0}"), FText::AsNumber(BrowserIdx + 1));
}

void FOasisContentBrowserSingleton::ToggleShowToolbarButton()
{
	const bool bShowToolbarButton = !GetDefault<UOasisContentBrowserSettings>()->DisplayToolbarButton;
	GetMutableDefault<UOasisContentBrowserSettings>()->DisplayToolbarButton = bShowToolbarButton;
	...
	if (bShowToolbarButton)
	{
		...
		{
//			ToolbarExtender = MakeShareable(new FExtender);
			ToolbarExtension = ToolbarExtender->AddToolBarExtension("Settings", EExtensionHook::After, OasisContentBrowserCommands, FToolBarExtensionDelegate::CreateRaw(this, &FOasisContentBrowserSingleton::AddToolbarExtension));
...
FText FOasisContentBrowserSingleton::GetContentBrowserTabLabel(int32 BrowserIdx)
{
#if ECB_WIP_MULTI_INSTANCES
...
#endif
	{
		return LOCTEXT("SpyEngineBrowserTabName", "SpyEngine");
	}
}
...
FOasisContentBrowserPluginInfo::FOasisContentBrowserPluginInfo()
{
	TSharedPtr<IPlugin> MyPlugin = IPluginManager::Get().FindPlugin(TEXT("SpyEngine"));
