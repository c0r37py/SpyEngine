...
#if ECB_LEGACY
bool FOasisPackageUtils::ParsePackage(struct FOasisAssetData& OutOasisAssetData)
{
	FString FileName = OutOasisAssetData.PackageFilePath.ToString();
	FString PackageName = OutOasisAssetData.PackageName.ToString();
	...
	if (!Linker)
	{
		OutOasisAssetData.InvalidReason = FOasisAssetData::EInvalidReason::FailedToLoadPackage;
		return false;
	}
	...
	{
		OutOasisAssetData.InvalidReason = FOasisAssetData::EInvalidReason::TextForamtNotSupported;
		return false;
	}
	...
	// Parsing package
	{
		// Version Info
		{
			OutOasisAssetData.FileVersionUE4 = Linker->UE4Ver();
			OutOasisAssetData.SavedByEngineVersion = Linker->Summary.SavedByEngineVersion;
			OutOasisAssetData.CompatibleWithEngineVersion = Linker->Summary.CompatibleWithEngineVersion;
		}
			...
			*Linker << AssetCount;
			...
			if (AssetCount > 0)
			{
				...
				*Linker << TagCount;
				...
				OutOasisAssetData.AssetName = FName(*ObjectPath);
				OutOasisAssetData.AssetClass = FName(*ObjectClassName);
				...
				OutOasisAssetData.TagsAndValues = FAssetDataTagMapSharedView(MoveTemp(NewTagsAndValues));
			}
		}
			...
			*Linker << ThumbnailCount;
			...
			if (ThumbnailCount > 0)
			{
				...
				*Linker << ObjectPath;
				...
				if (OutOasisAssetData.AssetClass == NAME_None && bHaveValidClassName)
				{
					OutOasisAssetData.AssetClass = FName(*ObjectClassName);
				}

				const bool bHaveValidAssetName = !ObjectPath.IsEmpty();
				if (OutOasisAssetData.AssetName == NAME_None && bHaveValidAssetName)
				{
					OutOasisAssetData.AssetName = FName(*ObjectPath);
				}
				...
				*Linker << FileOffset;
				...
				{
					OutOasisAssetData.bHasThumbnail = true;
				}
			}
		}
		...
		// Dependencies
		{
			...
			TSet<FName>& DependentPackages = OutOasisAssetData.HardDependentPackages;
		...
		{
			OutOasisAssetData.SoftPackageReferencesCount = Linker->Summary.SoftPackageReferencesCount;
		}
	}
...
bool FOasisPackageUtils::LoadThumbnail(const FOasisAssetData& InOasisAssetData, FObjectThumbnail& InOutThumbnail)
{
	bool bThumbnailLoaded = false;

	FString FileName = InOasisAssetData.PackageFilePath.ToString();
	FString PackageName = InOasisAssetData.PackageName.ToString();
...
void FOasisPackageUtils::LoadSoftReferences(const FOasisAssetData& InOasisAssetData, TArray<FName>& OutSoftPackageReferenceList)
{
	ECB_LOG(Display, TEXT("[LoadSoftReferences] Soft Package References Count: %10i"), InOasisAssetData.SoftPackageReferencesCount);

	if (InOasisAssetData.SoftPackageReferencesCount <= 0)
	{
		return;
	}
...
bool FOasisPackageUtils::ParsePackageWithPackageReader(struct FOasisAssetData& OutAssetData)
{
	...
	FOasisPackageReader PackageReader;
	FOasisPackageReader::EOpenPackageResult ErrorCode;
	if (!PackageReader.OpenPackageFile(PackageFilePath, &ErrorCode))
	{
		FOasisAssetData::EInvalidReason InvalidReason = FOasisAssetData::EInvalidReason::Unknown;

		switch (ErrorCode)
		{
			case FOasisPackageReader::EOpenPackageResult::FailedToLoad	:
			ECB_LOG(Display, TEXT("Failed to load %s, VersionTooOld!"), *PackageFilePath);
			InvalidReason = FOasisAssetData::EInvalidReason::PackageFileFailedToLoad;
			break;
			case FOasisPackageReader::EOpenPackageResult::VersionTooOld:
				ECB_LOG(Display, TEXT("Failed to open %s, VersionTooOld!"), *PackageFilePath);
				InvalidReason = FOasisAssetData::EInvalidReason::PackageFileVersionEmptyOrTooOld;
				break;
			case FOasisPackageReader::EOpenPackageResult::VersionTooNew:
				ECB_LOG(Display, TEXT("Failed to open %s, VersionTooNew!"), *PackageFilePath);
				InvalidReason = FOasisAssetData::EInvalidReason::PackageFileOrCustomVersionTooNew;
			break; 
			case FOasisPackageReader::EOpenPackageResult::NoLoader:
				ECB_LOG(Display, TEXT("Failed to open %s, NoLoader!"), *PackageFilePath);
				break;
			case FOasisPackageReader::EOpenPackageResult::MalformedTag:
				ECB_LOG(Display, TEXT("Failed to open %s, MalformedTag!"), *PackageFilePath);
				InvalidReason = FOasisAssetData::EInvalidReason::PackageFileMalformed;
				break;
			
			case FOasisPackageReader::EOpenPackageResult::CustomVersionMissing:
				ECB_LOG(Display, TEXT("Failed to open %s, CustomVersionMissing!"), *PackageFilePath);
				InvalidReason = FOasisAssetData::EInvalidReason::PackageFileCustomVersionMissing;
				break;
			default:
				InvalidReason = FOasisAssetData::EInvalidReason::Unknown;
	...
	if (bCooked)
	{
		...
		OutAssetData.InvalidReason = FOasisAssetData::EInvalidReason::CookedPackageNotSupported;
...
bool FOasisPackageUtils::LoadThumbnailWithPackageReader(const FOasisAssetData& InAssetData, FObjectThumbnail& InOutThumbnail)
{
...
void FOasisPackageUtils::UnloadPackage(const TArray<FString>& InFilePaths, TArray<UPackage*>* OutUnloadPackage)
{
...
void FOasisPackageUtils::GetDirectories(const FString& InBaseDirectory, TArray<FString>& OutPathList)
{
...
void FOasisPackageUtils::GetDirectoriesRecursively(const FString& InBaseDirectory, TArray<FString>& OutPathList)
{
	struct Local
	{
		static void GetChildDirectories(const TArray<FString>& InBaseDirs, TArray<FString>& OutPathList)
		{
			for (const FString& BaseDir : InBaseDirs)
			{
				...
				FOasisPackageUtils::GetDirectories(BaseDir, ChildDirs);
...
#if ECB_LEGACY
void FOasisPackageUtils::GetDirectories(const FString& InBaseDirectory, TArray<FString>& OutPathList, bool bResursively)
{
	double GetAllDirectoriesStartTime = FPlatformTime::Seconds();
...
void FOasisPackageUtils::GetAllPackages(const FString& InRootDirectory, TArray<FString>& OutFileList, bool bResursively)
{
...
void FOasisPackageUtils::GetAllPackages(const FName& InRootDirectory, TArray<FName>& OutFileList)
{
	struct FPackageVisitor : public IPlatformFile::FDirectoryVisitor
	{
		virtual bool Visit(const TCHAR* FilenameOrDirectory, bool bIsDirectory) override
...
bool FOasisPackageUtils::HasPackages(const FString& InDirectory, bool bResursively)
{
	struct FPackageVisitor : public IPlatformFile::FDirectoryVisitor
	{
		virtual bool Visit(const TCHAR* FilenameOrDirectory, bool bIsDirectory) override
...
bool FOasisPackageUtils::DoesPackageExist(const FString PackageName)
{
...
void FOasisPackageUtils::GeneratePackageReport(FLinkerLoad* Linker, uint32 InfoFlags, bool bHideOffsets)
{
	check(Linker);
