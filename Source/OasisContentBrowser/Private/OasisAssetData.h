...
#include "OasisFeedbackContextEditor.h"
#include "HAL/FeedbackContextAnsi.h"
#include "Serialization/BufferArchive.h"

#include "OasisAssetData.generated.h"
...
{
	const FString ExtTempPackagePath(TEXT("/Temp/SpyEngine"));
	const FString ExtSandboxPackagePath(TEXT("/SpyEngine/Sandbox"));
...
public:
	...
	FOasisAssetData(const FString& InPackageFilePath, bool bDelayParse = false);
	...
	FOasisAssetData(const UObject* InAsset, bool bAllowBlueprintClass = false);

	/** Constructor taking a package name. For support wrapping an internal package */
	FOasisAssetData(const FName& InPackageName);
...
public: // AssetData compatible interface
	...
	bool operator==(const FOasisAssetData& Other) const
	{
	...
	static FString& GetSpyEngineTempDir();
...
FORCEINLINE uint32 GetTypeHash(const FOasisAssetData& AssetData)
{
	return GetTypeHash(AssetData.PackageFilePath);
}

struct FOasisAssetDataUtil
{
#if 0
	...
	static bool SyncToContentBrowser(const FOasisAssetData& InAssetData);
};

namespace FExtAssetCoreUtil
{
	class FExtAssetFeedbackContext : public FOasisFeedbackContextEditor
	{
	public:
		/**
		 * cache the cancel result as FOasisFeedbackContextEditor::ReceivedUserCancel seems clears the result right away
		 */
...
struct FExtAssetDependencyWalker
{
	static EDependencyNodeStatus GatherDependencies(const FOasisAssetData& InAssetData, const FString& InAssetContentDir, const FOasisAssetData::EContentType& InAssetContentType, FExtAssetDependencyInfo& InOuDependencyInfo, bool bShowProgess = false);

private:

	static EDependencyNodeStatus GatherDependencies(const FOasisAssetData& InAssetData, const FString& InAssetContentDir, const FOasisAssetData::EContentType& InAssetContentType, FExtAssetDependencyInfo& InOuDependencyInfo, int32 Level, FScopedSlowTask* SlowTaskPtr = nullptr);
	...
	static EDependencyNodeStatus GatherPackageDependencies(const FName& DependPackageName, FExtAssetDependencyInfo& InOuDependencyInfo
		, const FString& InAssetContentDir, const FOasisAssetData::EContentType& InAssetContentType
...
public:
	...
	void CacheNewAsset(FOasisAssetData* InAssetToCache);

	void ReCacheParsedAsset(FOasisAssetData* InAssetToCache);
...
private:
	...
	void SetAssetDatas(TArrayView<FOasisAssetData> AssetDatas);
	void SerializeDependencyInfos(FArchive& Ar);
	void SerializeMisc(FArchive& Ar);

	TArray<FOasisAssetData*> PreallocatedAssetDataBuffers;
#endif
public:
	...
	TMap<FName, TArray<FOasisAssetData*> > CachedAssetsByFolder;
	...
	TMap<FName, FOasisAssetData*> CachedAssetsByFilePath;

	/** Assets been cached but not yet parsed, subset of  CachedAssetsByFilePath */
	TMap<FName, FOasisAssetData*> CachedUnParsedAssets;
	...
	TMap<FName, TArray<FOasisAssetData*> > CachedAssetsByClass;

	/** The map of asset tag to asset data for assets saved to disk */
	TMap<FName, TArray<FOasisAssetData*> > CachedAssetsByTag;
	...
	TMap<FName, FOasisAssetData::EContentType> CachedAssetContentType; // ContentRoot - ContentType
...
private:
	void CacheValidAssetInfo(FOasisAssetData* InAssetToCache);
...
{
	FExtAssetContentRootGatherResult(const FName& InAssetContentRootDir, const FName& InAssetContentRootHostDir, const FName& InAssetContentRootConfigDir, FOasisAssetData::EContentType InAssetContentType)
	...
	FOasisAssetData::EContentType AssetContentType;
};
...
struct FExtAssetGatherResult
{
	...
	{}
	TMap<FName, FOasisAssetData> ParsedAssets;

	void AddReuslt(const FName& InFilePath, const FOasisAssetData& InParsedAsset);
	bool HasResult() const;
	void Reset();
	const TMap<FName, FOasisAssetData>& GetParsedAssets() const { return ParsedAssets; };
};
...
public:
	...
	DECLARE_EVENT_TwoParams(FExtAssetRegistry, FAssetGatheredEvent, const FOasisAssetData& /*GatheredAsset*/, int32 /*Left*/);
	FAssetGatheredEvent& OnAssetGathered() { return AssetGatheredEvent; }

	DECLARE_EVENT_OneParam(FExtAssetRegistry, FAssetUpdatedEvent, const FOasisAssetData& /*UpdatedAssets*/);
	FAssetUpdatedEvent& OnAssetUpdated() { return AssetUpdatedEvent; }
	void BroadcastAssetUpdatedEvent(const FOasisAssetData& /*UpdatedAssets*/);
...
public:
	...
	bool GetAssets(const struct FARFilter& InFilter, TArray<FOasisAssetData>& OutAssetData);

	/** Trims items out of the asset data list that do not pass the supplied filter */
	void RunAssetsThroughFilter(TArray<FOasisAssetData>& AssetDataList, const FARFilter& Filter) const;
...
public:
	...
	static FPrimaryAssetId ExtractPrimaryAssetIdFromFakeAssetData(const FOasisAssetData& InAssetData);

	static void ExtractAssetIdentifiersFromAssetDataList(const TArray<FOasisAssetData>& AssetDataList, TArray<FExtAssetIdentifier>& OutAssetIdentifiers);
...
public:
	...
	/** Get root content folders */
	bool QueryRootContentPathInfo(const FString& InRootContentPath, FText* OutDisplayName = nullptr, FOasisAssetData::EContentType* OutContentType = nullptr, FString* OutAssetContentRoot = nullptr) const;

	/** Get relative path to any root path, return false if not match any */
	bool ParseAssetContentRoot(const FString& InFilePath, FString& OutRelativePath, FString& OutRootPath, FOasisAssetData::EContentType& OutAssetContentType, TSet<FName>* InAllDependencies = nullptr);

	/** Cache found asset content dir */
	void AddAssetContentRoot(const FString& InAssetContentRootDir, const FString& InAssetContentRootHostDir, const FString& ConfigDir, FOasisAssetData::EContentType InContentType);
	...
	FOasisAssetData::EContentType GetAssetContentRootContentType(const FString& InAssetContentRoot) const;
	...
	FExtAssetDependencyInfo GetOrCacheAssetDependencyInfo(const FOasisAssetData& InAssetData, bool bShowProgess = false);

	/** Get cached asset dependency info. */
	const FExtAssetDependencyInfo* GetCachedAssetDependencyInfo(const FOasisAssetData& InAssetData) const;
	bool RemoveCachedAssetDependencyInfo(const FOasisAssetData& InAssetData);

	bool IsCachedDependencyInfoInValid(const FOasisAssetData& InAssetData, bool bTreatSoftErrorAsInvalid = false) const;

	/** Get or cache an assets by file path. */
	FOasisAssetData* GetOrCacheAssetByFilePath(const FName& InFilePath, bool bDelayParse = false);
	
	/** Gets assets by directory, cache them if not already. */
	TMap<FName, TArray<FOasisAssetData*>>& GetOrCacheAssetsByFolder(const FName& InDirectory, bool bDelayParse = false);

	/** Gets assets by asset class, cache them if not already. */
	TMap<FName, TArray<FOasisAssetData*>>& GetOrCacheAssetsByClass(const TSet<FName>& InPaths, bool bRecursively);

	/** Get cached asset by file path. */
	FOasisAssetData* GetCachedAssetByFilePath(const FName& InFilePath);

	/** Get cached asset by package name. */
	FOasisAssetData* GetCachedAssetByPackageName(const FName& InPackageName);

	/** Gets cached assets by directory. */
	TArray<FOasisAssetData*> GetCachedAssetsByFolder(const FName& InDirectory);

	/** Gets cached assets by asset class. */
	TArray<FOasisAssetData*> GetCachedAssetsByClass(const FName& InClass);
	...
	TOptional<FAssetImportInfo> ExtractAssetImportInfo(const FOasisAssetData& AssetData) const;
...
{
	//static void ImportAssets(const TArray<FOasisAssetData>& InAssetDatas, const FExtAssetImportSetting& ImportSetting);
};
...
{
	static void ImportAssets(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& ImportSetting);
	static void ImportAssetsByFilePaths(const TArray<FString>& InAssetFilePaths, const FUAssetImportSetting& ImportSetting);
	static void ImportAssetsToFolderPackagePath(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& ImportSetting, const FString& InDestPackagePath);
	static void ImportAssetsWithPathPicker(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& ImportSetting);

	static void ImportProjectFolderColors(const FString& InRootOrHost);

	static void ExportAssets(const TArray<FOasisAssetData>& InAssetDatas);

	static void ZipUpSourceAssets(const TArray<FOasisAssetData>& InAssetDatas);
...
private:
	static void DoImportAssets(const TArray<FOasisAssetData>& InAssetDatas, const FUAssetImportSetting& ImportSetting);
...
{
	static bool ValidateDependency(const TArray<FOasisAssetData*>& InAssetDatas, FString* OutValidateResultPtr = nullptr, bool bShowProgess = false, EDependencyNodeStatus* OutAssetStatus = nullptr);
	static bool ValidateDependency(const TArray<FOasisAssetData>& InAssetDatas, FString* OutValidateResultPtr = nullptr, bool bShowProgess = false, EDependencyNodeStatus* OutAssetStatus = nullptr);
	static void InValidateDependency(const TArray<FOasisAssetData>& InAssetDatas);
};
...
private:
	bool DoGatherAssetByFilePath(const FName& InFilePath, FOasisAssetData& OutOasisAssetData) const;
